...
每个 SDK Web API 验收脚本必须输出统一的人工可读报告：

```text
UIAutoma Web API 测试
API     : uiautoma.web.<对象>.<api>
页面    : <测试 URL>
进度     状态    测试项                  测试结果
───────  ──────  ─────────────────────  ─────────────────────────────
01/NN    [通过]  <case_id>              <期望与实测摘要>
────────────────────────────────────────────────────────────────────────
测试通过 · N/N 通过 · 退出码 0
```

- 每个用例单独一行，至少覆盖合同、环境准备、核心场景、边界/异常和清理。
- `[通过]` 使用 ANSI 绿色，`[失败]` 使用 ANSI 红色，状态标签后恢复终端颜色。
- 汇总必须显示通过数、总数和退出码；退出码 `0` 表示全通过，`1` 表示存在 FAIL，`2` 表示存在 BLOCKED。
- 每行结果必须同时说明测试目标、期望值和实测摘要，不能只打印单一 `PASS`。

## 测试结束后的文档更新清单

每个 SDK Web API 完成脚本运行和人工复测后，按以下顺序更新测试侧文档：

1. `web/evidence/<api>.md`：记录合同、被测 worktree/commit、靶场与元素库、真实结果、期望值来源、独立确证、边界和复测命令。
2. `web/evidence/artifacts/<api>_<yyyymmdd>.txt`：保存本轮原始输出、通过数、退出码和复测命令。
3. `web/web api 清单.md`：更新该 API 的源码签名、实现状态和测试计数；源码不存在的 API 标记为不存在，不得保留旧签名。
4. `index.md`：同步 API 状态、测试脚本路径、证据路径和计数；与清单中的数字保持一致。
5. `AGENTS.md`：只有测试流程、输出格式或测试边界发生变化时才更新，不为单个 API 重复追加普通结果。

完成判据：脚本、证据、artifact、清单和总索引中的 API 名称、状态、计数与实际运行结果一致；源码仓库的公开文档和产品侧测试文件通过 Issue 交接，测试仓库不直接修改。
