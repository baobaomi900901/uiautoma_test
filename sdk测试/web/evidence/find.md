...
| 库中第二个元素 | 通过，同样可定位 |
| **多命中元素 → 歧义拒绝** | 通过，`AmbiguousElementError`（"元素匹配结果不唯一"，0.014s） |
...
本文件先前版本声明「本次所用元素库没有任何同名元素，无法构造歧义场景」——该判断**有误**。
实测逐一遍历库中 70 个元素（`timeout=0`）后确认：
...
带 `_相似元素` 后缀的元素即多命中元素，例如
`web靶场_表单测试_ant_radio_label_相似元素` → 3 个节点（男 / 女 / 其他）、
`web靶场_表单测试_原生_select多选_options_相似元素` → 4 个（北京 / 上海 / 广州 / 深圳）。

因此 `AmbiguousElementError` 已用**同一个元素的路径匹配多个 DOM 节点**这一真实场景覆盖，
`find_all()` 对该元素返回 3 个元素（见 `find_all.md`）。
