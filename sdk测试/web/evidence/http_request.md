...
**新基线**：`main@c101caa9`（原证据绑在 `dbe9e015`）。全量复跑结果 **41/41，退出码 0**。

**边界裁定（产品负责人 2026-09-18）**：测试侧**一律不使用地址形态探测**，因此：

- 删除 `connect_timeout_aborts_hanging`：原先用不可路由保留地址 `10.255.255.1` 制造
  「连接永不完成」以验证 `connect_timeout`。该语义现由回显服务的 `/delay/4`（响应头未到即中止）
  用例覆盖（`connect_timeout_bounds_headers`，0.8~3.0s 窗口）。
- 404 用例的路径由自造文件名改为**随机不存在路径**（标准靶场宿主上的真实 404），
  避免出现任何自造页面名。

用例数因此由 42 变为 **41**。第三方回显服务（`httpbin.org`）仍用于请求/响应真值的独立确证
（请求头、请求体、`/drip` 分块等）；它不属于自建靶场，但如需一并去除，则「请求真值」这一维度
在静态靶场上无法独立确证，需要产品负责人明确取舍。
