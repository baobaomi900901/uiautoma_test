...
**VERIFIED：29/29 通过 + 1 项如实刻画，退出码 0**（**连续 3 次完整运行全绿**）。
...
- **`scheduled`（默认，本次实测）**：测试侧用 `execute_javascript` 决定「何时」发生转变——
...
- **`delayed-page`（靶场自计时页，待部署）**：`public/delayed-element.html` 每 1.5s 交替
  插入/移除 `#delayed-target`，由页面自己计时，测试侧零调度。当前该页 **404**，
  `--transition-source delayed-page` 会如实记为 `BLOCKED`（退出码 2），`auto` 则自动回退 `scheduled`。

> 部署该页时请注意：**必须复用同一个 DOM 节点**（先 `createElement` 一次并保存引用，
> 之后只用 `appendChild` / `remove` 反复挂载），否则会被「已知边界 1」的节点身份语义挡住。
...
原始产物：[artifacts/wait_appear_20260915.txt](artifacts/wait_appear_20260915.txt)

```powershell
uv run .\web\test_web_browser_wait_appear.py                        # auto：探测自计时页，不可用则回退调度式
uv run .\web\test_web_browser_wait_appear.py --transition-source scheduled
uv run .\web\test_web_browser_wait_appear.py --transition-source delayed-page   # 未部署时如实 BLOCKED（退出码 2）
...
- **`wait_disappear()`**：同族另一 API，未测。
- **`delayed-page` 分支（靶场自计时页面）**：页面尚未部署，仅验证了 `BLOCKED` 路径；
  部署后需补跑并把该分支作为 canonical 场景。
