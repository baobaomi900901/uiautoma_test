...
import test_web_element_find_by_css_form as runner
...
class Root:
    def __init__(self, scope="root", wrong_multiple=False, syntax_trace="invalid_css_selector"):
        self.scope = scope
        self.wrong_multiple = wrong_multiple
        self.syntax_trace = syntax_trace

    def find_by_css(self, css_selector, *, timeout=10):
        ...
        raise AssertionError(f"Unexpected test selector: {css_selector!r}")


class CssRunnerTests(unittest.TestCase):
    def snapshot(self):
        return {
            ...
            'multiple_count': 20, 'missing_count': 0, 'self_count': 0, 'password_input_count': 1,
        }
    ...
    def run_cases(self, root):
        ...
        return runner.run_find_by_css_cases(root, SimpleNamespace(timeout=0, empty_timeout=0), seeds)
    ...
    def test_unrelated_protocol_failure_cannot_count_as_invalid_css(self):
        rows = self.run_cases(Root(syntax_trace="page_runtime_probe_timeout"))
        row = next(row for row in rows if row["case_id"] == "invalid_css_syntax")
        self.assertEqual(row["status"], "FAIL")
    ...
    def test_preparation_failure_cleans_partial_resources(self):
        with TemporaryDirectory() as temp:
            ...
            self.assertFalse(list(Path(temp).glob("uiautoma-element-css-*")))

    def test_cleanup_failure_is_not_swallowed(self):
        with TemporaryDirectory() as temp:
            copy = Path(temp) / "uiautoma-element-css-owned"
            copy.mkdir()
