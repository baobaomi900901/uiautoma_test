...
`document.querySelectorAll('input').length` 与 `#root` 的唯一性，再与该 API 返回的列表
长度比对；「不存在」用每次运行随机生成的 id，无需靶场预留类名。

与 `find_by_xpath()` 的关键差异：本 API 返回 `list`，**未匹配时返回空列表而不抛异常**；
但**语法非法时同样抛 `RpcProtocolError`**（不返回空列表）。
...
def run(args):
    ...
    try:
        ...
        results.append(list_case(
            page, "multi_count_matches_dom", f"{MULTI_XPATH} 多命中（应与 DOM 数量一致）",
            lambda: page.find_all_by_xpath(MULTI_XPATH, timeout=args.timeout), expected_count=multi_count))

        # 目标用例 2：唯一命中返回长度为 1 的列表
        results.append(list_case(
            page, "unique_list_of_one", f"{UNIQUE_XPATH} 唯一命中",
            lambda: page.find_all_by_xpath(UNIQUE_XPATH, timeout=args.timeout), expected_count=1))

        # 目标用例 3-4：不存在的表达式
        results.append(list_case(
            page, "not_found_empty_waits", "不存在的 id（timeout=3）",
            lambda: page.find_all_by_xpath(absent_xpath, timeout=3), expected_count=0, min_elapsed=2.5))
        results.append(list_case(
            page, "not_found_timeout_zero_fast", "不存在的 id（timeout=0）",
            lambda: page.find_all_by_xpath(absent_xpath, timeout=0), expected_count=0, max_elapsed=1.0))

        # 目标用例 5：timeout=0 且存在
        results.append(list_case(
            page, "timeout_zero_present", f"{UNIQUE_XPATH}（timeout=0）",
            lambda: page.find_all_by_xpath(UNIQUE_XPATH, timeout=0), expected_count=1))

        # 目标用例 6：语法非法 → RpcProtocolError（不返回空列表），页面仍可用
        results.append(expect_raises(
            lambda: page.find_all_by_xpath("///", timeout=3),
        ...
        results.append(expect_raises(
            lambda: closed_page.find_all_by_xpath(UNIQUE_XPATH, timeout=1), NoCurrentPackageError,
