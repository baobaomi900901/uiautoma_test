"""uiautoma.web.WebElement.check() 持久化真实浏览器测试运行器。

导入本模块不会连接 Runtime、访问靶场或操作浏览器。真实场景打开唯一 Chrome
``form-controls`` 页面并连接指定元素库；目标断言直接调用 ``WebElement.check(...)``。

当前靶场包含原生 HTML 与 Ant Design 两套复选框。现状下公开签名可通过，但真实
选中态验收未通过（见证据与 GitHub issue）；脚本如实记录 FAIL，不以 VERIFIED 宣称。
"""
...
DEFAULT_HTML_LABEL_NAME = "checkbox_html_input"
DEFAULT_ANT_LABEL_NAME = "checkbox_组件_label"
DEFAULT_ANT_INPUT_NAME = "checkbox_组件_input"
DEFAULT_SUBMIT_ELEMENT_NAME = "提交_html"
DEFAULT_RESET_ELEMENT_NAME = "重置_html"
RUN_QUERY_KEY = "uiautoma_element_check_run"
...
EXPECTED_KINDS = {
    ...
    "delay_after": inspect.Parameter.POSITIONAL_OR_KEYWORD,
}
    ...
    try:
        ...
        return (
            _result(
                ...
                "已打开本次唯一 check 测试页面" if passed else "check 测试页面创建结果不完整",
                elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
...
def check_contract() -> dict[str, Any]:
    try:
        signature = inspect.signature(WebElement.check)
        ...
        return_ok = signature.return_annotation in {None, type(None), "None"} or str(
            signature.return_annotation
        ) in {"None", "NoneType", "<class 'NoneType'>"}
...
def preflight_target(target_url: str, timeout: float) -> dict[str, Any]:
    ...
    try:
        ...
        return _result(
            "target_preflight",
            "PASS" if passed else "BLOCKED",
            "check 测试靶场可访问" if passed else "check 测试靶场未返回成功状态",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
...
def preflight_element_library(
    ...
    try:
        ...
        return _result(
            ...
            "元素库包含 check 场景所需库项" if all_ok else "元素库缺少 check 场景库项或 PageUrl/选择器不匹配",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
    ...
    try:
        ...
        return (
            _result(
                ...
                "PASS" if passed else "FAIL",
                "已连接元素库并绑定 check 场景元素" if passed else "元素库连接或元素绑定结果不符合预期",
                elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
                package_opened=True,
...
def _reset_form(reset_element: WebElement) -> None:
    reset_element.click(simulative=False, delay_after=0.2)


def _submit_hobbies(submit_element: WebElement) -> object:
    clipboard.set_text(f"uiautoma_check_sentinel_{uuid.uuid4().hex}")
    submit_element.click(simulative=False, delay_after=0.35)
    try:
        payload = json.loads(clipboard.get_text())
    except Exception:  # noqa: BLE001
        return None
    return payload.get("hobbies") if isinstance(payload, dict) else None


def _expect_not_checkable(element: WebElement, case_id: str) -> dict[str, Any]:
    started = time.perf_counter()
    try:
        element.check(mode="check", delay_after=0.2)
        return _result(
            case_id,
            "FAIL",
            "期望 element_not_checkable，但 check() 未抛错",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
            target_name=str(element.name or ""),
        )
    except Exception as exc:  # noqa: BLE001
        trace = _safe_trace(exc)
        # 现状：label 上调用 check 会得到 element_not_checkable。
        # 这是阻塞 VERIFIED 的已知产品缺口，脚本记为 FAIL（不是环境 BLOCKED）。
        matched = trace == "element_not_checkable" or "element_not_checkable" in str(exc)
        return _result(
            case_id,
            "FAIL",
            "label 目标 element_not_checkable（已知缺口，阻塞 VERIFIED）"
            if matched
            else "check 调用失败且不是预期的 element_not_checkable",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
            expected_trace="element_not_checkable",
            matched_not_checkable=matched,
            target_name=str(element.name or ""),
            **_error_fields(exc),
        )


def run_html_input_check_case(
    page: WebBrowser,
    html_label: WebElement,
    ...
    try:
        page.activate()
        time.sleep(0.1)
        _reset_form(reset_element)
        children = html_label.children()
        if not children:
            return _result(
                "html_input_check_form_state",
                "FAIL",
                "checkbox_html_input 无子 input，无法继续验收",
                elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
            )
        target = children[0]
        call_started = time.perf_counter()
        ret = target.check(mode="check", delay_after=0.25)
        check_elapsed_ms = round((time.perf_counter() - call_started) * 1000, 1)
        raw = target.last_result.raw if target.last_result is not None else {}
        hobbies = _submit_hobbies(submit_element)
        print(
            f"html child check() -> {ret!r}; raw.checked={raw.get('checked')!r}; hobbies={hobbies!r}",
            file=sys.stderr,
            flush=True,
        )
        return_none_ok = ret is None
        engine_checked_ok = raw.get("checked") is True
        form_ok = hobbies == ["旅行"] or hobbies == "旅行"
        # 目标：真实表单状态更新。引擎 raw.checked=True 但 hobbies 仍为 null → FAIL。
        passed = return_none_ok and form_ok
        return _result(
            "html_input_check_form_state",
            "PASS" if passed else "FAIL",
            "原生 HTML input check 后表单 hobbies 已更新"
            if passed
            else "原生 HTML input check 返回成功但表单 hobbies 未更新（受控组件缺口）",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
            check_elapsed_ms=check_elapsed_ms,
            return_none_ok=return_none_ok,
            engine_checked_ok=engine_checked_ok,
            form_hobbies_ok=form_ok,
            hobbies=hobbies,
            resolved_xpath=str(raw.get("resolved_xpath") or ""),
        ...
        return _result(
            "html_input_check_form_state",
            status,
            "html input check 被环境阻塞" if status == "BLOCKED" else "html input check 调用失败",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
            **_error_fields(exc),
        )


def run_ant_input_check_case(
    page: WebBrowser,
    ant_input: WebElement,
    ant_label: WebElement,
    reset_element: WebElement,
) -> dict[str, Any]:
    started = time.perf_counter()
    try:
        ...
        time.sleep(0.1)
        _reset_form(reset_element)
        ant_input = page.find(str(ant_input.name or DEFAULT_ANT_INPUT_NAME), timeout=8)
        call_started = time.perf_counter()
        ret = ant_input.check(mode="check", delay_after=0.25)
        check_elapsed_ms = round((time.perf_counter() - call_started) * 1000, 1)
        raw = ant_input.last_result.raw if ant_input.last_result is not None else {}
        label = page.find(str(ant_label.name or DEFAULT_ANT_LABEL_NAME), timeout=8)
        label_class = str(label.get_attribute("class") or "")
        class_ok = "ant-checkbox-wrapper-checked" in label_class
        print(
            f"ant input check() -> {ret!r}; raw.checked={raw.get('checked')!r}; class={label_class!r}",
            file=sys.stderr,
            flush=True,
        )
        return_none_ok = ret is None
        engine_checked_ok = raw.get("checked") is True
        passed = return_none_ok and class_ok
        return _result(
            "ant_input_check_ui_state",
            "PASS" if passed else "FAIL",
            "Ant input check 后 label 出现 checked class"
            if passed
            else "Ant input check 返回成功但 UI checked class 未出现（受控组件缺口）",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
            check_elapsed_ms=check_elapsed_ms,
            return_none_ok=return_none_ok,
            engine_checked_ok=engine_checked_ok,
            label_class_ok=class_ok,
            label_class=label_class,
            resolved_xpath=str(raw.get("resolved_xpath") or ""),
            target_name=str(ant_input.name or ""),
        )
    except Exception as exc:  # noqa: BLE001
        status = _error_status(exc)
        return _result(
            "ant_input_check_ui_state",
            status,
            "ant input check 被环境阻塞" if status == "BLOCKED" else "ant input check 调用失败",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
            **_error_fields(exc),
        )
...
def _report(args: argparse.Namespace, results: Sequence[dict[str, Any]]) -> tuple[dict[str, Any], int]:
    ...
    excluded = [
        ...
        "cef",
        "auto",
        "mode=uncheck / mode=toggle 的完整状态矩阵",
        "delay_after=1 耗时验收",
        "以 click() 代替 check() 的变通验收（对照实验，非目标 API）",
    ]
    return (
        {
            ...
            "status": status,
            "exit_code": exit_code,
            "lifecycle_hint": "READY_FOR_LIVE",
            "mode": args.mode,
            "profile_directory": args.profile_directory,
            "contract_only": bool(args.contract_only),
            "html_label_name": args.html_label_name,
            "ant_label_name": args.ant_label_name,
            "ant_input_name": args.ant_input_name,
            "results": list(results),
            "excluded": excluded,
            "open_defects": [
                "label 目标 element_not_checkable（未解析关联 input）",
                "check() 仅写 DOM checked，React/Ant 受控复选框真实状态不更新",
            ],
        },
        exit_code,
    )
    ...
    try:
        ...
        if scrub_result["status"] == "PASS":
            ...
            if page_result["status"] == "PASS" and page is not None:
                ...
                html_label = found.get(args.html_label_name)
                ant_label = found.get(args.ant_label_name)
                ant_input = found.get(args.ant_input_name)
                submit_element = found.get(args.submit_element_name)
                reset_element = found.get(args.reset_element_name)
                if (
                    library_result["status"] == "PASS"
                    and html_label is not None
                    and ant_label is not None
                    and ant_input is not None
                    and submit_element is not None
                    and reset_element is not None
                ):
                    results.append(_expect_not_checkable(html_label, "html_label_not_checkable"))
                    results.append(_expect_not_checkable(ant_label, "ant_label_not_checkable"))
                    results.append(
                        run_html_input_check_case(
                            page,
                            html_label,
                            reset_element,
                            submit_element,
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="运行 uiautoma.web.WebElement.check() 持久化真实浏览器测试。"
    )
    parser.add_argument(
        "--mode",
        default="chrome",
    ...
    parser.add_argument(
        ...
        help=f"check 测试靶场，默认 {DEFAULT_TARGET_URL}",
    )
    parser.add_argument(
        "--element-library",
        default=str(DEFAULT_ELEMENT_LIBRARY),
        type=Path,
        help="元素库目录；默认 tests/SDK/web/web测试元素库",
    )
    parser.add_argument("--html-label-name", default=DEFAULT_HTML_LABEL_NAME)
    parser.add_argument("--ant-label-name", default=DEFAULT_ANT_LABEL_NAME)
    parser.add_argument("--ant-input-name", default=DEFAULT_ANT_INPUT_NAME)
    parser.add_argument("--submit-element-name", default=DEFAULT_SUBMIT_ELEMENT_NAME)
    parser.add_argument("--reset-element-name", default=DEFAULT_RESET_ELEMENT_NAME)
