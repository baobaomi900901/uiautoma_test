"""uiautoma.web.WebElement.child_at() 持久化真实浏览器测试运行器。

导入本模块不会连接 Runtime、访问靶场或操作浏览器。真实场景打开唯一 Chrome
页面并连接指定元素库；先对 ``九宫格_中`` 调用 ``parent()`` 取得父级，再对父级
直接调用 ``WebElement.child_at(index)``，打印指定子元素并确认类型。
"""
...
DEFAULT_SEED_ELEMENT_NAME = "九宫格_中"
DEFAULT_PROFILE_DIRECTORY = "Default"
RUN_QUERY_KEY = "uiautoma_element_child_at_run"
...
# 与 children() 实测顺序对齐：0=左上 … 4=中 … 8=右下
CHILD_AT_CASES: tuple[dict[str, Any], ...] = (
    {
        "case_id": "child_at_0_top_left",
        "index": 0,
        "expected_name_substr": "topLeft",
    },
    {
        "case_id": "child_at_4_center",
        "index": 4,
        "expected_name_substr": "center",
    },
    {
        "case_id": "child_at_8_bottom_right",
        "index": 8,
        "expected_name_substr": "bottomRight",
    },
)
    ...
    try:
        ...
        return (
            _result(
                ...
                "已打开本次唯一按索引子元素测试页面" if passed else "按索引子元素测试页面创建结果不完整",
                elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
...
def check_contract() -> dict[str, Any]:
    try:
        signature = inspect.signature(WebElement.child_at)
        ...
        return_ok = ann in {WebElement, "WebElement", "'WebElement'"} or str(ann) in {
            "WebElement",
            "'WebElement'",
        }
        # index 无默认值
        index_required_ok = parameters["index"].default is inspect.Parameter.empty
        passed = order_ok and defaults_ok and kinds_ok and return_ok and index_required_ok
        return _result(
            "api_contract",
            "PASS" if passed else "FAIL",
            ...
            index_required_ok=index_required_ok,
...
def preflight_target(target_url: str, timeout: float) -> dict[str, Any]:
    ...
    try:
        ...
        return _result(
            "target_preflight",
            "PASS" if passed else "BLOCKED",
            "按索引子元素测试靶场可访问" if passed else "按索引子元素测试靶场未返回成功状态",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
...
def preflight_element_library(
    ...
    try:
        ...
        return _result(
            ...
            "PASS" if passed else "BLOCKED",
            "元素库包含种子元素 九宫格_中" if passed else "元素库缺少种子元素 九宫格_中",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
    ...
    try:
        ...
        return (
            _result(
                ...
                "PASS" if passed else "FAIL",
                "已连接元素库并绑定种子元素 九宫格_中" if passed else "元素库连接或元素绑定结果不符合预期",
                elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
                package_opened=True,
...
def _element_brief(element: WebElement) -> dict[str, Any]:
    raw = element.raw if isinstance(element.raw, dict) else {}
    xpath = str(
        raw.get("xpath")
        or raw.get("Characteristics")
        or raw.get("characteristics")
        or raw.get("resolved_xpath")
        or ""
    )
    return {
        "repr": str(element),
...
def run_child_at_cases(page: WebBrowser, seed: WebElement) -> list[dict[str, Any]]:
    results: list[dict[str, Any]] = []
    try:
        page.activate()
        time.sleep(0.1)
        parent = seed.parent()
        parent_ok = isinstance(parent, WebElement)
        parent_brief = _element_brief(parent) if parent_ok else {}
        print(f"parent={parent}", file=sys.stderr, flush=True)
        if not parent_ok:
            return [
                _result(
                    "child_at_parent_setup",
                    "FAIL",
                    "无法取得种子元素父级，跳过 child_at",
                    parent_ok=False,
                )
            ]

        for case in CHILD_AT_CASES:
            case_id = str(case["case_id"])
            index = int(case["index"])
            expected_substr = str(case["expected_name_substr"])
            started = time.perf_counter()
            try:
                call_started = time.perf_counter()
                child = parent.child_at(index)
                elapsed_ms = round((time.perf_counter() - call_started) * 1000, 1)
                print(f"child_at({index})={child}", file=sys.stderr, flush=True)
                brief = _element_brief(child)
                type_ok = isinstance(child, WebElement)
                name_ok = expected_substr.casefold() in str(brief.get("name") or "").casefold()
                identity_ok = bool(brief.get("id") or brief.get("name") or brief.get("xpath"))
                passed = type_ok and name_ok and identity_ok
                results.append(
                    _result(
                        case_id,
                        "PASS" if passed else "FAIL",
                        f"child_at({index}) 返回 WebElement 并已打印"
                        if passed
                        else f"child_at({index}) 返回值不符合预期",
                        elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
                        child_at_elapsed_ms=elapsed_ms,
                        index=index,
                        expected_name_substr=expected_substr,
                        return_type_ok=type_ok,
                        name_substr_ok=name_ok,
                        child_identity_ok=identity_ok,
                        seed_id=str(seed.id or ""),
                        seed_name=str(seed.name or ""),
                        parent=parent_brief,
                        child=brief,
                    )
                )
                ...
                results.append(
                    _result(
                        case_id,
                        status,
                        f"child_at({index}) 被环境阻塞"
                        if status == "BLOCKED"
                        else f"child_at({index}) 调用失败",
                        elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
                        index=index,
                        **_error_fields(exc),
                    )
        ...
        return [
            _result(
                "child_at_via_parent",
                status,
                "child_at 场景被环境阻塞" if status == "BLOCKED" else "child_at 场景失败",
                **_error_fields(exc),
            )
        ]
        ...
        if scrub_result["status"] == "PASS":
            ...
            if page_result["status"] == "PASS" and page is not None:
                ...
                if library_result["status"] == "PASS" and target is not None:
                    results.extend(run_child_at_cases(page, target))
    except Exception as exc:  # noqa: BLE001
        results.append(
            _result(
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="运行 uiautoma.web.WebElement.child_at() 持久化真实浏览器测试。"
    )
    parser.add_argument(
        "--mode",
        default="chrome",
    ...
    parser.add_argument(
        ...
        help=f"按索引子元素测试靶场，默认 {DEFAULT_TARGET_URL}",
    )
    parser.add_argument(
        "--element-library",
        type=Path,
        default=DEFAULT_ELEMENT_LIBRARY,
        help="包含种子元素九宫格_中的元素库目录。",
    )
    parser.add_argument(
        "--seed-element-name",
        default=DEFAULT_SEED_ELEMENT_NAME,
        help=f"种子元素名称，默认 {DEFAULT_SEED_ELEMENT_NAME}。",
    )
