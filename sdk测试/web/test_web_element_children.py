"""uiautoma.web.WebElement.children() 持久化真实浏览器测试运行器。

导入本模块不会连接 Runtime、访问靶场或操作浏览器。真实场景打开唯一 Chrome
页面并连接指定元素库；先对 ``九宫格_中`` 调用 ``parent()`` 取得父级，再对父级
直接调用无参 ``WebElement.children()``，打印子元素列表并确认类型。
"""
...
RUN_QUERY_KEY = "uiautoma_element_children_run"
EXPECTED_MIN_CHILDREN = 9
    ...
    try:
        ...
        return (
            _result(
                ...
                "已打开本次唯一子元素测试页面" if passed else "子元素测试页面创建结果不完整",
                elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
...
def check_contract() -> dict[str, Any]:
    try:
        signature = inspect.signature(WebElement.children)
        ...
        return_ok = ann_text in {
            "list[WebElement]",
            "list['WebElement']",
            'list["WebElement"]',
            "typing.List[WebElement]",
            "List[WebElement]",
        } or (isinstance(ann, type) and False)
...
def preflight_target(target_url: str, timeout: float) -> dict[str, Any]:
    ...
    try:
        ...
        return _result(
            "target_preflight",
            "PASS" if passed else "BLOCKED",
            "子元素测试靶场可访问" if passed else "子元素测试靶场未返回成功状态",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
...
def run_children_case(page: WebBrowser, seed: WebElement) -> dict[str, Any]:
    started = time.perf_counter()
    try:
        page.activate()
        time.sleep(0.1)
        ...
        print(children, file=sys.stderr, flush=True)
        for index, child in enumerate(children if isinstance(children, list) else []):
            print(f"children[{index}]={child}", file=sys.stderr, flush=True)

        list_ok = isinstance(children, list)
        items_ok = list_ok and all(isinstance(item, WebElement) for item in children)
        count = len(children) if list_ok else 0
        count_ok = count >= EXPECTED_MIN_CHILDREN
        child_briefs = [_element_brief(item) for item in children] if items_ok else []
        contains_seed_name = any(
            "center" in str(item.get("name") or "").casefold()
            or "center" in str(item.get("xpath") or "").casefold()
            or "data-position" in str(item.get("xpath") or "").casefold()
            for item in child_briefs
        )
        # 九宫格中心格应出现在父级 children 中（名称/xpath 含 center 或 data-position）。
        contains_center_ok = contains_seed_name
        passed = parent_ok and list_ok and items_ok and count_ok and contains_center_ok
        return _result(
            "children_via_parent",
            "PASS" if passed else "FAIL",
            "children 返回 WebElement 列表并已打印" if passed else "children 返回值不符合预期",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
            children_elapsed_ms=elapsed_ms,
            parent_ok=parent_ok,
            return_list_ok=list_ok,
            items_are_web_element_ok=items_ok,
            children_count=count,
            expected_min_children=EXPECTED_MIN_CHILDREN,
            children_count_ok=count_ok,
            contains_center_ok=contains_center_ok,
        ...
        return _result(
            "children_via_parent",
            status,
            "children 被环境阻塞" if status == "BLOCKED" else "children 调用失败",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
            **_error_fields(exc),
        )
        ...
        if scrub_result["status"] == "PASS":
            ...
            if page_result["status"] == "PASS" and page is not None:
                ...
                if library_result["status"] == "PASS" and target is not None:
                    results.append(run_children_case(page, target))
    except Exception as exc:  # noqa: BLE001
        results.append(
            _result(
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="运行 uiautoma.web.WebElement.children() 持久化真实浏览器测试。"
    )
    parser.add_argument(
        "--mode",
        default="chrome",
    ...
    parser.add_argument(
        ...
        help=f"子元素测试靶场，默认 {DEFAULT_TARGET_URL}",
    )
    parser.add_argument(
        "--element-library",
        type=Path,
