"""uiautoma.web.WebElement.click() 持久化真实浏览器测试运行器。

导入本模块不会连接 Runtime、访问靶场或操作浏览器。真实场景打开唯一 Chrome
页面并连接指定元素库；目标断言只直接调用 ``WebElement.click()``，每次点击后
...
DEFAULT_CLICK_ELEMENT_NAME = "单击触发"
DEFAULT_RIGHT_ELEMENT_NAME = "右键触发"
DEFAULT_GRID_ELEMENT_NAME = "按钮_九宫格"
DEFAULT_COPY_ELEMENT_NAME = "复制最近的一条记录"
DEFAULT_PROFILE_DIRECTORY = "Default"
RUN_QUERY_KEY = "uiautoma_element_click_run"
...
BACKGROUND_CLICK_CASES: tuple[dict[str, Any], ...] = (
    {
        "case_id": "click_left_keys_none",
        "element": "left",
        "kwargs": {"button": "left", "simulative": False, "keys": "none", "delay_after": 0.2},
        "expected_button_id": "btn-click-target",
        "expected_event_type": "click",
        "expected_keys": "none",
    ...
    {
        "case_id": "click_left_keys_ctrl",
        "element": "left",
        "kwargs": {"button": "left", "simulative": False, "keys": "ctrl", "delay_after": 0.2},
        "expected_button_id": "btn-click-target",
        "expected_event_type": "click",
        "expected_keys": "ctrl",
        "expected_source": "background",
    ...
    {
        "case_id": "click_left_keys_alt",
        "element": "left",
        "kwargs": {"button": "left", "simulative": False, "keys": "alt", "delay_after": 0.2},
        "expected_button_id": "btn-click-target",
        "expected_event_type": "click",
        "expected_keys": "alt",
        "expected_source": "background",
    ...
    {
        "case_id": "click_left_keys_shift",
        "element": "left",
        "kwargs": {"button": "left", "simulative": False, "keys": "shift", "delay_after": 0.2},
        "expected_button_id": "btn-click-target",
        "expected_event_type": "click",
        "expected_keys": "shift",
        "expected_source": "background",
    ...
    {
        "case_id": "click_left_keys_win",
        "element": "left",
        "kwargs": {"button": "left", "simulative": False, "keys": "win", "delay_after": 0.2},
        "expected_button_id": "btn-click-target",
        "expected_event_type": "click",
        "expected_keys": "win",
        "expected_source": "background",
    ...
    {
        "case_id": "click_delay_after",
        "element": "left",
        "kwargs": {"button": "left", "simulative": False, "keys": "none", "delay_after": 1},
        "expected_button_id": "btn-click-target",
        "expected_event_type": "click",
        "expected_keys": "none",
        "expected_source": "background",
        "expected_trusted": False,
        "min_elapsed_ms": 900.0,
        "dismiss_context_menu": False,
    },
)

SIMULATIVE_CLICK_CASES: tuple[dict[str, Any], ...] = (
    {
        "case_id": "click_simulative_left_keys_none",
        "element": "left",
    ...
    {
        "case_id": "click_simulative_left_keys_ctrl",
        "element": "left",
        "kwargs": {"button": "left", "simulative": True, "keys": "ctrl", "delay_after": 0.2},
        "expected_button_id": "btn-click-target",
        "expected_event_type": "click",
        "expected_keys": "ctrl",
        "expected_source": "simulative",
    ...
    {
        "case_id": "click_simulative_left_keys_alt",
        "element": "left",
        "kwargs": {"button": "left", "simulative": True, "keys": "alt", "delay_after": 0.2},
        "expected_button_id": "btn-click-target",
        "expected_event_type": "click",
        "expected_keys": "alt",
        "expected_source": "simulative",
    ...
    {
        "case_id": "click_simulative_left_keys_shift",
        "element": "left",
        "kwargs": {"button": "left", "simulative": True, "keys": "shift", "delay_after": 0.2},
        "expected_button_id": "btn-click-target",
        "expected_event_type": "click",
        "expected_keys": "shift",
        "expected_source": "simulative",
    ...
    {
        "case_id": "click_simulative_left_keys_win",
        "element": "left",
        "kwargs": {"button": "left", "simulative": True, "keys": "win", "delay_after": 0.2},
        "expected_button_id": "btn-click-target",
        "expected_event_type": "click",
        "expected_keys": "win",
        "expected_source": "simulative",
    ...
    {
        "case_id": "click_simulative_right_keys_none",
        "element": "right",
        "kwargs": {"button": "right", "simulative": True, "keys": "none", "delay_after": 0.2},
        "expected_button_id": "btn-rightclick-target",
        "expected_event_type": "contextmenu",
        "expected_keys": "none",
        "expected_source": "simulative",
        "expected_trusted": True,
        "min_elapsed_ms": 0.0,
...
# 九宫格各方位：offset 取“向中心 20px”（中心点自身为 0,0）
# expected_click_position_prefix 对齐靶场页面文案，而非 SDK 枚举原名。
_GRID_ANCHOR_TOWARD_CENTER: tuple[tuple[str, int, int, str], ...] = (
    ("topLeft", 20, 20, "topLeft"),
    ("topCenter", 0, 20, "top-"),
    ("topRight", -20, 20, "topRight"),
    ("middleLeft", 20, 0, "left-"),
    ("middleCenter", 0, 0, "center-"),
    ("middleRight", -20, 0, "right-"),
    ("bottomLeft", 20, -20, "bottomLeft"),
    ("bottomCenter", 0, -20, "bottom-"),
    ("bottomRight", -20, -20, "bottomRight"),
)

GRID_ANCHOR_CLICK_CASES: tuple[dict[str, Any], ...] = tuple(
    {
        "case_id": f"click_simulative_anchor_{part}",
        "element": "grid",
        "kwargs": {
            "button": "left",
            "simulative": True,
            "keys": "none",
            "delay_after": 0.2,
            "anchor": (part, offset_x, offset_y),
        },
        "expected_button_id": "position-grid-panel",
        "expected_event_type": "click",
        "expected_keys": "none",
        "expected_source": "simulative",
        "expected_trusted": True,
        "expected_click_position_prefix": position_prefix,
        "expect_offsets_present": True,
        "min_elapsed_ms": 0.0,
        "dismiss_context_menu": False,
    }
    for part, offset_x, offset_y, position_prefix in _GRID_ANCHOR_TOWARD_CENTER
)

SIMULATIVE_CLICK_CASES = SIMULATIVE_CLICK_CASES + GRID_ANCHOR_CLICK_CASES
...
def _public_record_fields(record: dict[str, Any] | None, expected_button_id: str) -> dict[str, Any]:
    ...
    return {
        ...
        "click_position": str(record.get("clickPosition") or ""),
        "offset_x": record.get("offsetX"),
        "offset_y": record.get("offsetY"),
    }
...
def preflight_target(target_url: str, timeout: float) -> dict[str, Any]:
    ...
    try:
        ...
        return _result(
            ...
            "点击测试靶场可访问" if passed else "点击测试靶场未返回成功状态",
...
def preflight_element_library(
    library_dir: Path,
    click_element_name: str,
    right_element_name: str,
    grid_element_name: str,
    ...
    try:
        ...
        click_matches = _library_named_matches(payload, click_element_name)
        right_matches = _library_named_matches(payload, right_element_name)
        grid_matches = _library_named_matches(payload, grid_element_name)
        copy_matches = _library_named_matches(payload, copy_element_name)
        click_page_ok, click_selector_ok = _library_item_ok(click_matches, target_url)
        right_page_ok, right_selector_ok = _library_item_ok(right_matches, target_url)
        grid_page_ok, grid_selector_ok = _library_item_ok(grid_matches, target_url)
        copy_page_ok, copy_selector_ok = _library_item_ok(copy_matches, target_url)
        passed = (
            library_dir.is_dir()
            and elements_file.is_file()
            and click_page_ok
            and click_selector_ok
            and right_page_ok
            and right_selector_ok
            and grid_page_ok
            and grid_selector_ok
            and copy_page_ok
        ...
        return _result(
            ...
            "PASS" if passed else "BLOCKED",
            "元素库包含单击、右键、九宫格与复制记录元素" if passed else "元素库缺少单击、右键、九宫格或复制记录元素",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
            library_dir_exists=library_dir.is_dir(),
            elements_json_exists=elements_file.is_file(),
            click_named_element_count=len(click_matches),
            right_named_element_count=len(right_matches),
            grid_named_element_count=len(grid_matches),
            copy_named_element_count=len(copy_matches),
            click_page_url_matches=click_page_ok,
            right_page_url_matches=right_page_ok,
            grid_page_url_matches=grid_page_ok,
            copy_page_url_matches=copy_page_ok,
            click_selector_present=click_selector_ok,
            right_selector_present=right_selector_ok,
            grid_selector_present=grid_selector_ok,
    ...
    try:
        ...
        return (
            _result(
                "owned_page_setup",
                "PASS" if passed else "BLOCKED",
                "已打开本次唯一点击测试页面" if passed else "点击测试页面创建结果不完整",
                elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
...
def connect_library_and_find_elements(
    ...
    try:
        ...
        click_element = page.find(click_element_name, timeout=element_timeout)
        right_element = page.find(right_element_name, timeout=element_timeout)
        grid_element = page.find(grid_element_name, timeout=element_timeout)
        copy_element = page.find(copy_element_name, timeout=element_timeout)
        click_name_ok = str(click_element.name or "") == click_element_name
        right_name_ok = str(right_element.name or "") == right_element_name
        grid_name_ok = str(grid_element.name or "") == grid_element_name
        ...
        return (
            _result(
                ...
                "PASS" if passed else "FAIL",
                "已连接元素库并绑定单击、右键、九宫格与复制元素"
                if passed
                else "元素库连接或元素绑定结果不符合预期",
                elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
                package_opened=True,
                web_element_count=package.web_count,
                click_element_name_ok=click_name_ok,
                right_element_name_ok=right_name_ok,
                grid_element_name_ok=grid_name_ok,
...
def _copy_latest_record(copy_element: WebElement) -> tuple[dict[str, Any] | None, str]:
    sentinel = f"uiautoma_click_sentinel_{uuid.uuid4().hex}"
...
def run_click_case(
    ...
    min_elapsed_ms = float(case["min_elapsed_ms"])
    simulative = bool(kwargs.get("simulative"))
    target = elements[str(case["element"])]
    try:
        page.activate()
        time.sleep(0.1)
        call_started = time.perf_counter()
        returned = target.click(**kwargs)
        elapsed_ms = round((time.perf_counter() - call_started) * 1000, 1)
        return_is_none = returned is None
        if case.get("dismiss_context_menu"):
        ...
        delay_ok = elapsed_ms >= min_elapsed_ms
        position_prefix = str(case.get("expected_click_position_prefix") or "")
        if position_prefix:
            position_ok = str(fields["click_position"]).startswith(position_prefix)
        else:
            position_ok = True
        if case.get("expect_offsets_present"):
            offsets_ok = fields["offset_x"] is not None and fields["offset_y"] is not None
        else:
            offsets_ok = True
        passed = (
            return_is_none
        ...
        return _result(
            case_id,
            "PASS" if passed else "FAIL",
            f"{detail_prefix} click 与剪贴板记录符合预期"
            if passed
            else f"{detail_prefix} click 返回值或剪贴板记录不符合预期",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
            click_elapsed_ms=elapsed_ms,
            ...
            expected_click_position_prefix=position_prefix or None,
        ...
        return _result(
            case_id,
            status,
            f"{detail_prefix} click 被环境阻塞" if status == "BLOCKED" else f"{detail_prefix} click 调用失败",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
            simulative=simulative,
            button=kwargs.get("button"),
            **_error_fields(exc),
    ...
    return (
        {
            ...
            "click_element_name": args.click_element_name,
            "right_element_name": args.right_element_name,
            "grid_element_name": args.grid_element_name,
...
def run(args: argparse.Namespace) -> tuple[dict[str, Any], int]:
    ...
    results.append(
        preflight_element_library(
            ...
            args.click_element_name,
            args.right_element_name,
            args.grid_element_name,
        ...
        if scrub_result["status"] == "PASS":
            ...
            if page_result["status"] == "PASS" and page is not None:
                ...
                if (
                    ...
                    for case in BACKGROUND_CLICK_CASES:
                        results.append(run_click_case(page, elements, copy_element, case))
                        if results[-1]["status"] != "PASS":
                            break
                    if results[-1]["status"] == "PASS":
                        ...
                        if window_result["status"] == "PASS":
                            for case in SIMULATIVE_CLICK_CASES:
                                results.append(run_click_case(page, elements, copy_element, case))
                                if results[-1]["status"] != "PASS":
                                    break
    except Exception as exc:  # noqa: BLE001
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="运行 uiautoma.web.WebElement.click() 持久化真实浏览器测试。"
    )
    parser.add_argument(
        "--mode",
        default="chrome",
    ...
    parser.add_argument("--target-url", default=DEFAULT_TARGET_URL, help=f"点击测试靶场，默认 {DEFAULT_TARGET_URL}")
    parser.add_argument(
        "--element-library",
        type=Path,
        default=DEFAULT_ELEMENT_LIBRARY,
        help="包含点击与复制元素的元素库目录。",
    )
    parser.add_argument(
        "--click-element-name",
        default=DEFAULT_CLICK_ELEMENT_NAME,
        help=f"左键点击元素名称，默认 {DEFAULT_CLICK_ELEMENT_NAME}。",
    )
    parser.add_argument(
        "--right-element-name",
        default=DEFAULT_RIGHT_ELEMENT_NAME,
        help=f"右键点击元素名称，默认 {DEFAULT_RIGHT_ELEMENT_NAME}。",
    )
    parser.add_argument(
        "--grid-element-name",
        default=DEFAULT_GRID_ELEMENT_NAME,
        help=f"九宫格锚点元素名称，默认 {DEFAULT_GRID_ELEMENT_NAME}。",
    )
    parser.add_argument(
        "--copy-element-name",
    ...
    args.click_element_name = str(args.click_element_name or "").strip()
    args.right_element_name = str(args.right_element_name or "").strip()
    args.grid_element_name = str(args.grid_element_name or "").strip()
    args.copy_element_name = str(args.copy_element_name or "").strip()
    args.profile_directory = str(args.profile_directory or "").strip()
    if not args.click_element_name:
        parser.error("--click-element-name 不能为空")
    if not args.right_element_name:
        parser.error("--right-element-name 不能为空")
    if not args.grid_element_name:
        parser.error("--grid-element-name 不能为空")
