"""WebElement.clipboard_input() 原生表单完整验收。

流程：打开靶场 -> 核对输入/提交/重置元素 -> clipboard_input() -> 提交读取 JSON.text
...
def contract() -> str:
    ...
    expected = ("self", "text", "append", "focus_timeout", "delay_after",
                "send_key_delay", "click_before_input", "anchor",
                "input_check", "retry_times", "check_value")
    require(tuple(params) == expected, f"公开签名不符: {signature}")
    ...
    defaults = {"append": False, "focus_timeout": 1000, "delay_after": 1,
                "send_key_delay": 50, "click_before_input": True, "anchor": None,
                "input_check": False, "retry_times": 3, "check_value": ""}
    for name, default in defaults.items():
        require(params[name].kind is inspect.Parameter.KEYWORD_ONLY
                and params[name].default == default, f"{name} 默认值或参数种类不符")
    ...
    return "clipboard_input 公开签名、参数种类、默认值和 None 返回值符合合同"
...
def cases(base: str) -> list[dict[str, Any]]:
    ...
    return [
        {"id": "clipboard_basic", "steps": [(base, {"delay_after": 0.1})], "expected": base},
        {"id": "clipboard_special", "steps": [(SPECIAL_TEXT, {"delay_after": 0.1})], "expected": SPECIAL_TEXT},
        {"id": "clipboard_append", "steps": [
            (base[:half], {"append": False, "delay_after": 0.1}),
            (base[half:], {"append": True, "delay_after": 0.1}),
        ], "expected": base},
        {"id": "clipboard_input_check", "steps": [
            (base, {"input_check": True, "check_value": base, "retry_times": 1, "delay_after": 0.1}),
        ], "expected": base},
        {"id": "clipboard_focus_timeout_zero", "steps": [(base, {"focus_timeout": 0, "delay_after": 0.1})], "expected": base},
        {"id": "clipboard_send_key_delay_zero", "steps": [(base, {"send_key_delay": 0, "delay_after": 0.1})], "expected": base},
        {"id": "clipboard_click_before_input_false", "steps": [(base, {"click_before_input": False, "delay_after": 0.1})], "expected": base, "focus_first": True},
        {"id": "clipboard_anchor_string", "steps": [(base, {"anchor": "middle_center", "delay_after": 0.1})], "expected": base},
        {"id": "clipboard_anchor_tuple", "steps": [(base, {"anchor": ("top_left", 5, 5), "delay_after": 0.1})], "expected": base},
        {"id": "clipboard_anchor_dict", "steps": [(base, {"anchor": {"anchor": "bottom_right", "offset_x": -5, "offset_y": -5}, "delay_after": 0.1})], "expected": base},
        {"id": "clipboard_anchor_random", "steps": [(base, {"anchor": "random", "delay_after": 0.1})], "expected": base},
        {"id": "clipboard_delay_none", "steps": [(base, {"delay_after": None})], "expected": base},
        {"id": "clipboard_delay_positive", "steps": [(base, {"delay_after": 0.2})], "expected": base, "min_elapsed": 0.18},
    ]
...
def run_case(page: WebBrowser, item: dict[str, Any], timeout: float, element_timeout: float) -> str:
    ...
    for text_value, kwargs in item["steps"]:
        returned = elements["input"].clipboard_input(text_value, **kwargs)
    ...
    require(all(value is None for value in returned_values), f"clipboard_input 返回值异常: {returned_values!r}")
    ...
    return f"{detail}；提交方式={mode}；clipboard_input 调用 {elapsed * 1000:.1f}ms"
...
def main(argv=None):
    parser = argparse.ArgumentParser(description="WebElement.clipboard_input() 原生表单完整验收")
    ...
    try:
        ...
        if rows[-1]["status"] != "PASS":
            return print_report(args, rows, 1)
        page = package = None
        resources = {}
        try:
            ...
            # 页面已经打开后再做 HTTPS 预检；预检阻塞不阻止继续使用浏览器实测。
...
def print_report(args, rows, code, started=None):
    ...
    if args.json:
        print(json.dumps({"api": "uiautoma.web.WebElement.clipboard_input", "status": "PASS" if code == 0 else "FAIL", "exit_code": code, "results": rows}, ensure_ascii=False, indent=2))
        return code
    line("UIAutoma Web API 测试")
    line("API     : uiautoma.web.WebElement.clipboard_input")
    line(f"靶场模式: {args.target_variant}")
    line(f"页面    : {URL}")
    line(f"元素库  : {args.library}")
    line(f"测试元素: {INPUT_NAME}")
    line(f"提交元素: {SUBMIT_NAME}")
    line("参照    : clipboard_input 后点击原生提交按钮读取剪贴板 JSON，仅核对 payload.text")
