"""WebElement.find_all_by_css() 表单输入框独立测试。

本文件包含完整准备、用例、断言、输出及清理逻辑，仅依赖标准库和 uiautoma SDK。
运行函数：run_find_all_by_css_cases()；入口：main()。"""
...
GROUP_CSS = ', '.join(f'input#{dom_id}' for dom_id in INPUT_IDS.values())
...
DOM_ORACLE = """function (element, args) {
  ...
  return {
    ...
    all_inputs: Array.from(root.querySelectorAll('input')).map(node => ({
      id: node.getAttribute('id'), type: node.getAttribute('type'),
      role: node.getAttribute('role'), placeholder: node.getAttribute('placeholder')
    })),
    group_ids: Array.from(root.querySelectorAll(args.group)).map(node => node.id),
...
def contract():
    sig = inspect.signature(WebElement.find_all_by_css)
    ...
    return result('api_contract', 'PASS' if ok else 'FAIL',
                  'css_selector 必填；timeout 仅限关键字且默认 10；返回 list[WebElement]'
...
def validate_dom(snapshot):
    ...
    if (not isinstance(all_inputs, list) or len(all_inputs) != multiple
        ...
        raise AssertionError(f'全部 input 属性参照与数量不一致: {all_inputs!r}')
    group_ids = snapshot.get('group_ids')
    if not isinstance(group_ids, list) or Counter(group_ids) != Counter(INPUT_IDS.values()):
        raise AssertionError(f'分组选取应包含四个 Ant 输入框各一次，实际 {group_ids!r}')
    return snapshot


def run_find_all_by_css_cases(root, args, seeds, baseline):
    results = []
    for index, (label, css, name, wrapper) in enumerate(CSS_CASES):
        if index == 0:
            record_case(results, label, lambda css=css, name=name, wrapper=wrapper:
                        check_css_target(root.find_all_by_css(css), name, wrapper))
        else:
            record_case(results, label, lambda css=css, name=name, wrapper=wrapper:
                        check_css_target(root.find_all_by_css(css, timeout=args.timeout), name, wrapper))
    record_case(results, 'keyword_css', lambda:
                check_matches(root.find_all_by_css(css_selector=TEXT_CSS, timeout=args.timeout), [INPUT_IDS[TARGETS[0]]]))
    record_case(results, 'zero_timeout_hit', lambda:
                check_matches(root.find_all_by_css(TEXT_CSS, timeout=0), [INPUT_IDS[TARGETS[0]]]))

    def repeat():
        checks = [check_matches(root.find_all_by_css(TEXT_CSS, timeout=0), [INPUT_IDS[TARGETS[0]]]) for _ in range(3)]
        return '连续三次均为单项 WebElement 列表；' + checks[-1]
    record_case(results, 'repeat_read', repeat)
    record_case(results, 'multiple_matches', lambda:
                check_all_inputs(root.find_all_by_css('input', timeout=args.timeout), baseline['all_inputs']))
    record_case(results, 'grouped_selector', lambda:
                check_matches(root.find_all_by_css(GROUP_CSS, timeout=args.timeout), baseline['group_ids']))
    record_case(results, 'missing_zero', lambda:
                check_empty(lambda: root.find_all_by_css(MISSING_CSS, timeout=0)))
    record_case(results, 'missing_wait', lambda:
                check_empty(lambda: root.find_all_by_css(MISSING_CSS, timeout=args.empty_timeout),
                            min_elapsed=args.empty_timeout))
    record_case(results, 'scope_outside', lambda:
                check_empty(lambda: seeds[TARGETS[0]].find_all_by_css(EMAIL_CSS, timeout=0)))
    record_case(results, 'scope_excludes_self', lambda:
                check_empty(lambda: root.find_all_by_css('#shadow-form-content', timeout=0)))
    record_case(results, 'password_inner_scope', lambda:
                check_matches(seeds[PASSWORD_NAME].find_all_by_css('input', timeout=0), [INPUT_IDS[PASSWORD_NAME]]))

    # 语法错误的期望是当前 SDK 的精确映射，不接受其它 Runtime 故障作为通过。
    record_case(results, 'invalid_css_syntax', lambda:
                expect_exception(lambda: root.find_all_by_css('input[', timeout=0),
    ...
    for label, invalid in (('selector_none', None), ('selector_integer', 123),
        ...
        record_case(results, label, lambda invalid=invalid:
                    expect_exception(lambda: root.find_all_by_css(invalid, timeout=0), InvalidParamsError))
    record_case(results, 'invalid_timeout', lambda:
                expect_exception(lambda: root.find_all_by_css(TEXT_CSS, timeout=-2), InvalidParamsError))
    record_case(results, 'invalid_timeout_type', lambda:
                expect_exception(lambda: root.find_all_by_css(TEXT_CSS, timeout='bad'), InvalidParamsError))
    record_case(results, 'missing_selector', lambda:
                expect_exception(lambda: root.find_all_by_css(), TypeError))
    record_case(results, 'positional_timeout', lambda:
                expect_exception(lambda: root.find_all_by_css(TEXT_CSS, 1), TypeError))
    record_case(results, 'unknown_keyword', lambda:
                expect_exception(lambda: root.find_all_by_css(TEXT_CSS, unsupported=True), TypeError))
    return results
...
def cleanup(resources, results, temp_root: Path):
    ...
    if temp_dir is not None:
        def remove_copy():
            ...
            if resolved.parent != temp_root.resolve() or not resolved.name.startswith('uiautoma-element-all-css-'):
...
def run(args):
    ...
    try:
        ...
        temp_dir = Path(tempfile.mkdtemp(prefix='uiautoma-element-all-css-', dir=args.temp_root))
        ...
        prepared(f"5 个 CSS 各命中 1 项；全部 input={baseline['multiple_count']} 项；"
                 "分组 CSS 为四个 Ant 输入框；不存在选择器及根自身为 0 项；密码包装内 input=1")

        stage, started = 'find_all_by_css_cases', time.perf_counter()
        results.extend(run_find_all_by_css_cases(root, args, seeds, baseline))
    except Exception as exc:
...
def main(argv=None):
    import argparse
    parser = argparse.ArgumentParser(description='WebElement.find_all_by_css() 表单输入框专项验收')
    ...
    print('API     : uiautoma.web.WebElement.find_all_by_css', flush=True)
    print(f'页面    : {URL}', flush=True)
    print(f'元素库  : {args.library}', flush=True)
    print(f'SDK     : {uiautoma.__file__}', flush=True)
    if not args.contract_only:
        print('参照    : 元素库定位根容器；CSS 返回子树内全部匹配；原生 DOM 独立核对数量及属性', flush=True)
    started = time.perf_counter()
