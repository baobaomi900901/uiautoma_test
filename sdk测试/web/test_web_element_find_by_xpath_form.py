"""WebElement.find_by_xpath() 表单输入框独立测试。

本文件包含完整准备、用例、断言、输出及清理逻辑，仅依赖标准库和 uiautoma SDK。
运行函数：run_find_by_xpath_cases()；入口：main()。"""
...
DOM_ORACLE = """function (element, args) {
  ...
  return {
    ...
    multiple_count: evaluate('.//input', root).length,
...
def contract():
    sig = inspect.signature(WebElement.find_by_xpath)
    ...
    return result('api_contract', 'PASS' if ok else 'FAIL',
                  'xpath_selector 必填；timeout 仅限关键字且默认 10；返回唯一 WebElement'
...
def validate_dom(snapshot):
    ...
    return (f'5 个相对 XPath 各唯一命中；.//input 多项={multiple}；'
            '未命中与子树外各 0 项；. 返回根自身；密码 ./input 与 ./input/.. 各 1 项')


def run_find_by_xpath_cases(root, args, seeds):
    results = []
    for index, (label, xpath, name, wrapper) in enumerate(XPATH_CASES):
        if index == 0:
            record_case(results, label, lambda xpath=xpath, name=name, wrapper=wrapper:
                        check_xpath_target(root.find_by_xpath(xpath), name, wrapper))
        else:
            record_case(results, label, lambda xpath=xpath, name=name, wrapper=wrapper:
                        check_xpath_target(root.find_by_xpath(xpath, timeout=args.timeout), name, wrapper))
    record_case(results, 'keyword_xpath', lambda:
                check_element(root.find_by_xpath(xpath_selector=TEXT_XPATH, timeout=args.timeout), INPUT_IDS[TARGETS[0]]))
    record_case(results, 'zero_timeout_hit', lambda:
                check_element(root.find_by_xpath(TEXT_XPATH, timeout=0), INPUT_IDS[TARGETS[0]]))

    def repeat():
        checks = [check_element(root.find_by_xpath(TEXT_XPATH, timeout=0), INPUT_IDS[TARGETS[0]]) for _ in range(3)]
        return '连续三次分别核对 DOM id；' + checks[-1]
    record_case(results, 'repeat_read', repeat)
    record_case(results, 'ambiguous', lambda:
                expect_exception(lambda: root.find_by_xpath('.//input', timeout=0), AmbiguousElementError))
    record_case(results, 'missing_zero', lambda:
                expect_exception(lambda: root.find_by_xpath(MISSING_XPATH, timeout=0), ElementNotFoundError))
    record_case(results, 'missing_wait', lambda:
                expect_exception(lambda: root.find_by_xpath(MISSING_XPATH, timeout=args.empty_timeout),
                                 ElementNotFoundError, min_elapsed=args.empty_timeout))
    record_case(results, 'scope_outside', lambda:
                expect_exception(lambda: seeds[TARGETS[0]].find_by_xpath(EMAIL_XPATH, timeout=0), ElementNotFoundError))
    record_case(results, 'relative_excludes_self', lambda:
                expect_exception(lambda: root.find_by_xpath(DESCENDANT_SELF_XPATH, timeout=0), ElementNotFoundError))
    record_case(results, 'self_context', lambda:
                check_element(root.find_by_xpath('.', timeout=0), 'shadow-form-content'))
    record_case(results, 'password_direct_child', lambda:
                check_element(seeds[PASSWORD_NAME].find_by_xpath('./input', timeout=0), INPUT_IDS[PASSWORD_NAME]))
    record_case(results, 'parent_axis', lambda:
                check_saved_element(seeds[PASSWORD_NAME].find_by_xpath('./input/..', timeout=0), PASSWORD_NAME))

    # 当前源码快照的语法错误映射；其它 Runtime 错误不能替代该预期。
    record_case(results, 'invalid_xpath_syntax', lambda:
                expect_exception(lambda: root.find_by_xpath('.//[', timeout=0),
    ...
    for label, invalid in (('selector_none', None), ('selector_integer', 123),
        ...
        record_case(results, label, lambda invalid=invalid:
                    expect_exception(lambda: root.find_by_xpath(invalid, timeout=0), InvalidParamsError))
    record_case(results, 'invalid_timeout', lambda:
                expect_exception(lambda: root.find_by_xpath(TEXT_XPATH, timeout=-2), InvalidParamsError))
    record_case(results, 'invalid_timeout_type', lambda:
                expect_exception(lambda: root.find_by_xpath(TEXT_XPATH, timeout='bad'), InvalidParamsError))
    record_case(results, 'missing_selector', lambda:
                expect_exception(lambda: root.find_by_xpath(), TypeError))
    record_case(results, 'positional_timeout', lambda:
                expect_exception(lambda: root.find_by_xpath(TEXT_XPATH, 1), TypeError))
    record_case(results, 'unknown_keyword', lambda:
                expect_exception(lambda: root.find_by_xpath(TEXT_XPATH, unsupported=True), TypeError))
    return results
...
def cleanup(resources, results, temp_root: Path):
    ...
    if temp_dir is not None:
        def remove_copy():
            ...
            if resolved.parent != temp_root.resolve() or not resolved.name.startswith('uiautoma-element-xpath-'):
...
def run(args):
    ...
    try:
        ...
        temp_dir = Path(tempfile.mkdtemp(prefix='uiautoma-element-xpath-', dir=args.temp_root))
        ...
        stage, started = 'find_by_xpath_cases', time.perf_counter()
        results.extend(run_find_by_xpath_cases(root, args, seeds))
    except Exception as exc:
...
def main(argv=None):
    import argparse
    parser = argparse.ArgumentParser(description='WebElement.find_by_xpath() 表单输入框专项验收')
    ...
    print('API     : uiautoma.web.WebElement.find_by_xpath', flush=True)
    print(f'页面    : {URL}', flush=True)
    print(f'元素库  : {args.library}', flush=True)
    print(f'SDK     : {uiautoma.__file__}', flush=True)
    if not args.contract_only:
        print('参照    : 元素库定位根容器；相对 XPath；原生 document.evaluate 独立核对', flush=True)
    started = time.perf_counter()
