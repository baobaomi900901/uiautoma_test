"""uiautoma.web.WebElement.focus() 持久化真实浏览器测试运行器。

导入本模块不会连接 Runtime、访问靶场或操作浏览器。真实场景打开唯一 Chrome
页面并连接指定元素库；目标断言只直接调用 ``WebElement.focus()``（无入参），
每次聚焦后通过“复制最近的一条记录”写入剪贴板再对比结果。

本脚本仅覆盖元素库 ``focus`` 的默认无参调用。
"""
...
DEFAULT_FOCUS_ELEMENT_NAME = "focus"
DEFAULT_COPY_ELEMENT_NAME = "复制最近的一条记录"
DEFAULT_PROFILE_DIRECTORY = "Default"
RUN_QUERY_KEY = "uiautoma_element_focus_run"

# 公开实现额外暴露 keyword-only timeout；用户场景以无参 focus() 为准。
...
EXPECTED_FOCUS_EVENT_TYPE = "focus"
EXPECTED_FOCUS_KEYS = "-"
...
FOCUS_CASES: tuple[dict[str, Any], ...] = (
    {
        "case_id": "focus_default",
        "expected_button_id": "focus-target",
    },
)
...
def check_contract() -> dict[str, Any]:
    try:
        signature = inspect.signature(WebElement.focus)
...
def preflight_target(target_url: str, timeout: float) -> dict[str, Any]:
    ...
    try:
        ...
        return _result(
            ...
            "聚焦测试靶场可访问" if passed else "聚焦测试靶场未返回成功状态",
...
def preflight_element_library(
    ...
    try:
        ...
        focus_matches = _library_named_matches(payload, focus_element_name)
        copy_matches = _library_named_matches(payload, copy_element_name)
        focus_page_ok, focus_selector_ok = _library_item_ok(focus_matches, target_url)
        copy_page_ok, copy_selector_ok = _library_item_ok(copy_matches, target_url)
        passed = (
            library_dir.is_dir()
            and elements_file.is_file()
            and focus_page_ok
            and focus_selector_ok
            and copy_page_ok
        ...
        return _result(
            ...
            "PASS" if passed else "BLOCKED",
            "元素库包含 focus 与复制记录元素" if passed else "元素库缺少 focus 或复制记录元素",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
            library_dir_exists=library_dir.is_dir(),
            elements_json_exists=elements_file.is_file(),
            focus_named_element_count=len(focus_matches),
            copy_named_element_count=len(copy_matches),
            focus_page_url_matches=focus_page_ok,
            copy_page_url_matches=copy_page_ok,
            focus_selector_present=focus_selector_ok,
    ...
    try:
        ...
        return (
            _result(
                "owned_page_setup",
                "PASS" if passed else "BLOCKED",
                "已打开本次唯一聚焦测试页面" if passed else "聚焦测试页面创建结果不完整",
                elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
...
def connect_library_and_find_elements(
    ...
    try:
        ...
        focus_element = page.find(focus_element_name, timeout=element_timeout)
        copy_element = page.find(copy_element_name, timeout=element_timeout)
        focus_name_ok = str(focus_element.name or "") == focus_element_name
        copy_name_ok = str(copy_element.name or "") == copy_element_name
        passed = package.web_count > 0 and focus_name_ok and copy_name_ok
        return (
            _result(
                "element_library_setup",
                "PASS" if passed else "FAIL",
                "已连接元素库并绑定 focus 与复制元素"
                if passed
                else "元素库连接或元素绑定结果不符合预期",
                elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
                package_opened=True,
                web_element_count=package.web_count,
                focus_element_name_ok=focus_name_ok,
...
def _copy_latest_record(copy_element: WebElement) -> tuple[dict[str, Any] | None, str]:
    sentinel = f"uiautoma_focus_sentinel_{uuid.uuid4().hex}"
...
def run_focus_case(
    ...
    try:
        ...
        # focus 走 DOM 聚焦，点击记录通常为非真实鼠标、非 trusted。
        source_ok = _source_ok(str(fields["click_source"]), "background")
        keys_ok = str(fields["detected_keys"]) == EXPECTED_FOCUS_KEYS
        event_ok = str(fields["event_type"]) == EXPECTED_FOCUS_EVENT_TYPE
        trusted_ok = fields["is_trusted"] is False
        passed = (
            return_is_none
        ...
        return _result(
            case_id,
            "PASS" if passed else "FAIL",
            "focus 与剪贴板记录符合预期" if passed else "focus 返回值或剪贴板记录不符合预期",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
            focus_elapsed_ms=elapsed_ms,
            return_is_none=return_is_none,
            copy_mode=copy_mode,
            expected_button_id=expected_button_id,
            expected_event_type=EXPECTED_FOCUS_EVENT_TYPE,
            expected_keys=EXPECTED_FOCUS_KEYS,
        ...
        return _result(
            case_id,
            status,
            "focus 被环境阻塞" if status == "BLOCKED" else "focus 调用失败",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
            **_error_fields(exc),
        )
    ...
    return (
        {
            ...
            "focus_element_name": args.focus_element_name,
        ...
        if scrub_result["status"] == "PASS":
            ...
            if page_result["status"] == "PASS" and page is not None:
                ...
                if (
                    ...
                    and focus_element is not None
                    and copy_element is not None
                ):
                    for case in FOCUS_CASES:
                        results.append(
                            run_focus_case(page, focus_element, copy_element, case)
                        )
                        if results[-1]["status"] != "PASS":
                            break
    except Exception as exc:  # noqa: BLE001
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="运行 uiautoma.web.WebElement.focus() 持久化真实浏览器测试。"
    )
    parser.add_argument(
        "--mode",
        default="chrome",
    ...
    parser.add_argument(
        ...
        help=f"聚焦测试靶场，默认 {DEFAULT_TARGET_URL}",
    )
    parser.add_argument(
        "--element-library",
        type=Path,
        default=DEFAULT_ELEMENT_LIBRARY,
        help="包含聚焦与复制元素的元素库目录。",
    )
    parser.add_argument(
        "--focus-element-name",
        default=DEFAULT_FOCUS_ELEMENT_NAME,
        help=f"聚焦目标元素名称，默认 {DEFAULT_FOCUS_ELEMENT_NAME}。",
    )
    parser.add_argument(
        "--copy-element-name",
    ...
    args.focus_element_name = str(args.focus_element_name or "").strip()
    args.copy_element_name = str(args.copy_element_name or "").strip()
    args.profile_directory = str(args.profile_directory or "").strip()
    if not args.focus_element_name:
        parser.error("--focus-element-name 不能为空")
