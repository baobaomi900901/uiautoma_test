"""uiautoma.web.WebElement.get_all_attributes() 持久化真实浏览器测试运行器。

导入本模块不会连接 Runtime、访问靶场或操作浏览器。真实场景打开唯一 Chrome
``form-controls`` 页面并连接指定元素库；对 ``input元素`` 直接调用无参
``WebElement.get_all_attributes()``，核对返回 ``dict[str, str]`` 及关键属性。
"""
...
RUN_QUERY_KEY = "uiautoma_element_get_all_attributes_run"
...
# 目标 API 是无参 get_all_attributes()；公开合同返回 dict[str, str]。
    ...
    try:
        ...
        return (
            _result(
                ...
                "已打开本次唯一get_all_attributes 测试页面" if passed else "get_all_attributes 测试页面创建结果不完整",
                elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
...
def check_contract() -> dict[str, Any]:
    try:
        signature = inspect.signature(WebElement.get_all_attributes)
        ...
        ann_text = str(ann).replace(" ", "")
        return_ok = ann in {dict, "dict[str, str]", "Dict[str, str]"} or ann_text in {
            "dict[str,str]",
            "Dict[str,str]",
            "'dict[str,str]'",
        } or "dict[str,str]" in ann_text
...
def preflight_target(target_url: str, timeout: float) -> dict[str, Any]:
    ...
    try:
        ...
        return _result(
            "target_preflight",
            "PASS" if passed else "BLOCKED",
            "get_all_attributes 测试靶场可访问" if passed else "get_all_attributes 测试靶场未返回成功状态",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
...
def run_get_all_attributes_case(
    page: WebBrowser,
    ...
    try:
        ...
        attrs = input_element.get_all_attributes()
        elapsed_ms = round((time.perf_counter() - call_started) * 1000, 1)
        print(f"get_all_attributes()={attrs!r}", file=sys.stderr, flush=True)
        type_ok = isinstance(attrs, dict)
        str_values_ok = type_ok and all(
            isinstance(key, str) and isinstance(value, str) for key, value in attrs.items()
        )
        placeholder_ok = type_ok and attrs.get("placeholder") == "请输入文本"
        type_attr_ok = type_ok and attrs.get("type") == "text"
        has_id_ok = type_ok and bool(str(attrs.get("id") or "").strip())
        # 空 value 属性可能不出现在全部属性枚举中，不作为硬性键。
        passed = type_ok and str_values_ok and placeholder_ok and type_attr_ok and has_id_ok
        return _result(
            "get_all_attributes_input",
            "PASS" if passed else "FAIL",
            "get_all_attributes 返回 dict 且含关键属性" if passed else "get_all_attributes 返回不符合预期",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
            get_all_attributes_elapsed_ms=elapsed_ms,
            return_type_ok=type_ok,
            str_values_ok=str_values_ok,
            placeholder_ok=placeholder_ok,
            type_attr_ok=type_attr_ok,
            has_id_ok=has_id_ok,
            has_value_key=("value" in attrs) if type_ok else False,
            attribute_keys=sorted(attrs.keys()) if type_ok else [],
            attribute_count=len(attrs) if type_ok else 0,
            placeholder=str(attrs.get("placeholder")) if type_ok else "",
            type_attr=str(attrs.get("type")) if type_ok else "",
            reset_mode=reset_mode,
            target_id=str(input_element.id or ""),
        ...
        return _result(
            "get_all_attributes_input",
            status,
            "get_all_attributes 被环境阻塞" if status == "BLOCKED" else "get_all_attributes 调用失败",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
            **_error_fields(exc),
        )
...
def _report(args: argparse.Namespace, results: Sequence[dict[str, Any]]) -> tuple[dict[str, Any], int]:
    ...
    excluded = [
        ...
        "cef",
        "auto",
        "List[Tuple] 返回形态（公开 Web 合同为 dict）",
        "属性集合随页面样式变化的完整相等断言",
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="运行 uiautoma.web.WebElement.get_all_attributes() 持久化真实浏览器测试。"
    )
    parser.add_argument(
        "--mode",
        default="chrome",
    ...
    parser.add_argument(
        ...
        help=f"get_all_attributes 测试靶场，默认 {DEFAULT_TARGET_URL}",
    )
    parser.add_argument(
        "--element-library",
        type=Path,
