"""uiautoma.web.WebElement.get_select_options() 持久化真实浏览器测试运行器。

导入本模块不会连接 Runtime、访问靶场或操作浏览器。真实场景打开唯一 Chrome
``form-controls`` 页面并连接指定元素库；对 ``select_html`` 直接调用
``WebElement.get_select_options()``。当前版本显式暂不支持该 API。
"""
...
RUN_QUERY_KEY = "uiautoma_element_get_select_options_run"
...
# 目标 API：get_select_options() -> list[tuple[Any, ...]]
# 用户合同：List[Tuple]（选项文本，选项值，被选中状态）
# 当前实现直接抛 UnsupportedActionError（web.element.get_select_options）。

# 重置后 select_html（#form-controls-native-city）已知选项。
EXPECTED_OPTIONS_AFTER_RESET = (
    ("请选择城市", "", True),
    ("北京", "beijing", False),
    ("上海", "shanghai", False),
    ("广州", "guangzhou", False),
    ...
    try:
        ...
        return (
            _result(
                ...
                "已打开本次唯一 get_select_options 测试页面" if passed else "get_select_options 测试页面创建结果不完整",
                elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
        ...
        return (
            _result(
                "owned_page_setup",
                "BLOCKED",
                "无法打开本次 get_select_options 测试页面",
                elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
                profile_directory=profile_directory,
                existing_session_reused=existing_session_reused,
                profile_identity_fallback_used=profile_identity_fallback_used,
...
def check_contract() -> dict[str, Any]:
    try:
        signature = inspect.signature(WebElement.get_select_options)
        ...
        return_ok = (
            ...
            or "list[tuple" in ann_text.casefold()
            or "List[Tuple" in ann_text
        )
...
def preflight_target(target_url: str, timeout: float) -> dict[str, Any]:
    ...
    try:
        ...
        return _result(
            "target_preflight",
            "PASS" if passed else "BLOCKED",
            "get_select_options 测试靶场可访问" if passed else "get_select_options 测试靶场未返回成功状态",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
...
def _normalize_option_row(row: object) -> tuple[str, str, bool] | None:
    if not isinstance(row, (tuple, list)) or len(row) < 3:
        return None
    label, value, selected = row[0], row[1], row[2]
    return str(label), str(value), bool(selected)


def run_get_select_options_case(
    page: WebBrowser,
    ...
    try:
        ...
        try:
            ...
            return _result(
                "get_select_options_after_reset",
                status,
                "重置表单失败，无法开始 get_select_options",
                elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
                target_name=str(select_element.name or ""),
                **_error_fields(reset_exc),
            )
        call_started = time.perf_counter()
        ret = select_element.get_select_options()
        call_elapsed_ms = round((time.perf_counter() - call_started) * 1000, 1)
        print(f"get_select_options() -> {ret!r}", file=sys.stderr, flush=True)
        type_ok = isinstance(ret, list)
        rows = [_normalize_option_row(item) for item in ret] if type_ok else []
        rows_ok = type_ok and all(row is not None for row in rows)
        expected_ok = rows_ok and tuple(rows) == EXPECTED_OPTIONS_AFTER_RESET
        passed = type_ok and rows_ok and expected_ok
        return _result(
            "get_select_options_after_reset",
            "PASS" if passed else "FAIL",
            "get_select_options 返回选项列表符合预期"
            if passed
            else "get_select_options 返回值不符合预期",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
            get_select_options_elapsed_ms=call_elapsed_ms,
            return_type_ok=type_ok,
            rows_shape_ok=rows_ok,
            expected_options_ok=expected_ok,
            option_count=len(ret) if type_ok else 0,
            value=rows if rows_ok else (ret if type_ok else None),
            target_name=str(select_element.name or ""),
        )
    except Exception as exc:  # noqa: BLE001
        status = _error_status(exc)
        detail = "get_select_options 当前版本暂不支持（阻塞 VERIFIED）"
        if _safe_trace(exc) and _safe_trace(exc) != "web.element.get_select_options":
            detail = (
                "get_select_options 被环境阻塞"
                if status == "BLOCKED"
                else "get_select_options 调用失败"
            )
        if "暂不支持" in str(exc) or "unsupported" in str(exc).casefold():
            status = "FAIL"
            detail = "get_select_options 当前版本暂不支持（阻塞 VERIFIED）"
        print(
            f"get_select_options() raised {exc.__class__.__name__}: {exc}",
            file=sys.stderr,
            flush=True,
        )
        return _result(
    ...
    return (
        {
            ...
            "excluded": excluded,
            "open_defects": ["web.element.get_select_options unsupported"],
        },
        exit_code,
    )
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="运行 uiautoma.web.WebElement.get_select_options() 持久化真实浏览器测试。"
    )
    parser.add_argument(
        "--mode",
        default="chrome",
    ...
    parser.add_argument(
        ...
        help=f"get_select_options 测试靶场，默认 {DEFAULT_TARGET_URL}",
    )
    parser.add_argument(
        "--element-library",
        type=Path,
