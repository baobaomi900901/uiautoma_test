"""uiautoma.web.WebElement.get_selected_item() 持久化真实浏览器测试运行器。

导入本模块不会连接 Runtime、访问靶场或操作浏览器。真实场景打开唯一 Chrome
``form-controls`` 页面并连接指定元素库；对 ``select_html`` 用 ``select()``
切换选中态后，直接调用 ``WebElement.get_selected_item()``。当前版本显式暂不支持。
"""
...
RUN_QUERY_KEY = "uiautoma_element_get_selected_item_run"
...
# 目标 API：get_selected_item() -> list[str]
# 场景准备可用 select()（非目标 API）；当前实现直接抛 UnsupportedActionError。
    ...
    try:
        ...
        return (
            _result(
                ...
                "已打开本次唯一 get_selected_item 测试页面" if passed else "get_selected_item 测试页面创建结果不完整",
                elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
...
def check_contract() -> dict[str, Any]:
    try:
        signature = inspect.signature(WebElement.get_selected_item)
...
def preflight_target(target_url: str, timeout: float) -> dict[str, Any]:
    ...
    try:
        ...
        return _result(
            "target_preflight",
            "PASS" if passed else "BLOCKED",
            "get_selected_item 测试靶场可访问" if passed else "get_selected_item 测试靶场未返回成功状态",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
...
def _call_get_selected_item(select_element: WebElement) -> tuple[object | None, Exception | None, float]:
    call_started = time.perf_counter()
    try:
        ret = select_element.get_selected_item()
        return ret, None, round((time.perf_counter() - call_started) * 1000, 1)
    except Exception as exc:  # noqa: BLE001
        return None, exc, round((time.perf_counter() - call_started) * 1000, 1)
...
def _selected_result(
    *,
    ...
    if exc is not None:
        status = _error_status(exc)
        detail = "get_selected_item 当前版本暂不支持（阻塞 VERIFIED）"
        if _safe_trace(exc) and _safe_trace(exc) != "web.element.get_selected_item":
            detail = (
                "get_selected_item 被环境阻塞"
                if status == "BLOCKED"
                else "get_selected_item 调用失败"
            )
        ...
        return _result(
            ...
            detail,
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
            get_selected_item_elapsed_ms=call_elapsed_ms,
            expected=expected,
            target_name=str(select_element.name or ""),
            **_error_fields(exc),
        )

    print(f"{case_id}: get_selected_item() -> {ret!r}", file=sys.stderr, flush=True)
    type_ok = isinstance(ret, list)
    items_ok = type_ok and all(isinstance(item, str) for item in ret)
    expected_ok = items_ok and list(ret) == expected
    passed = type_ok and items_ok and expected_ok
    return _result(
        case_id,
        "PASS" if passed else "FAIL",
        "get_selected_item 返回当前选中项符合预期"
        if passed
        else "get_selected_item 返回值不符合预期",
        elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
        get_selected_item_elapsed_ms=call_elapsed_ms,
...
def run_get_selected_item_cases(
    page: WebBrowser,
    select_element: WebElement,
    ...
    try:
        ...
        return [
            _result(
                "get_selected_item_setup_reset",
                status,
                "重置表单失败，无法开始 get_selected_item",
                elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
                target_name=str(select_element.name or ""),
    ...
    ret, exc, elapsed = _call_get_selected_item(select_element)
    results.append(
        _selected_result(
            case_id="get_selected_item_after_reset",
            started=started,
            ret=ret,
            exc=exc,
            call_elapsed_ms=elapsed,
            expected=EXPECTED_SELECTED_AFTER_RESET,
    ...
    try:
        select_element.select("上海", mode="fuzzy", delay_after=0.2)
    except Exception as select_exc:  # noqa: BLE001
        status = _error_status(select_exc)
        results.append(
            _result(
                "get_selected_item_after_select_shanghai",
                status,
                "场景准备 select('上海') 失败，无法验收 get_selected_item",
                elapsed_ms=round((time.perf_counter() - started2) * 1000, 1),
                target_name=str(select_element.name or ""),
                **_error_fields(select_exc),
            )
        )
        return results

    ret2, exc2, elapsed2 = _call_get_selected_item(select_element)
    results.append(
        _selected_result(
            case_id="get_selected_item_after_select_shanghai",
            started=started2,
            ret=ret2,
            exc=exc2,
            call_elapsed_ms=elapsed2,
            expected=EXPECTED_SELECTED_AFTER_SELECT_SHANGHAI,
            select_element=select_element,
        )
    )
    return results
    ...
    return (
        {
            ...
            "excluded": excluded,
            "open_defects": [
                "web.element.get_selected_item unsupported",
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="运行 uiautoma.web.WebElement.get_selected_item() 持久化真实浏览器测试。"
    )
    parser.add_argument(
        "--mode",
        default="chrome",
    ...
    parser.add_argument(
        ...
        help=f"get_selected_item 测试靶场，默认 {DEFAULT_TARGET_URL}",
    )
    parser.add_argument(
        "--element-library",
        type=Path,
