"""uiautoma.web.WebElement.hover() 持久化真实浏览器测试运行器。

导入本模块不会连接 Runtime、访问靶场或操作浏览器。真实场景打开唯一 Chrome
页面并连接指定元素库；目标断言只直接调用 ``WebElement.hover()``，每次悬停后
通过“复制最近的一条记录”写入剪贴板再对比结果。

本脚本仅覆盖元素库 ``hover`` 的后台（非人工）与模拟人工悬停，以及 delay_after=1。
"""
...
DEFAULT_HOVER_ELEMENT_NAME = "hover"
DEFAULT_COPY_ELEMENT_NAME = "复制最近的一条记录"
DEFAULT_PROFILE_DIRECTORY = "Default"
RUN_QUERY_KEY = "uiautoma_element_hover_run"
...
EXPECTED_KINDS = {
    ...
    "simulative": inspect.Parameter.POSITIONAL_OR_KEYWORD,
    "delay_after": inspect.Parameter.POSITIONAL_OR_KEYWORD,
    "anchor": inspect.Parameter.POSITIONAL_OR_KEYWORD,
}
# 靶场点击记录对悬停写 eventType=hover，辅助键列显示为 "-"。
EXPECTED_HOVER_EVENT_TYPE = "hover"
EXPECTED_HOVER_KEYS = "-"
...
BACKGROUND_HOVER_CASES: tuple[dict[str, Any], ...] = (
    {
        "case_id": "hover_background",
        "kwargs": {"simulative": False, "delay_after": 0.2},
        "expected_button_id": "hover-target",
        "expected_source": "background",
        "expected_trusted": False,
        "min_elapsed_ms": 0.0,
    },
    {
        "case_id": "hover_delay_after",
        "kwargs": {"simulative": False, "delay_after": 1},
        "expected_button_id": "hover-target",
        "expected_source": "background",
        "expected_trusted": False,
...
SIMULATIVE_HOVER_CASES: tuple[dict[str, Any], ...] = (
    {
        "case_id": "hover_simulative_default",
...
def preflight_target(target_url: str, timeout: float) -> dict[str, Any]:
    ...
    try:
        ...
        return _result(
            ...
            "悬停测试靶场可访问" if passed else "悬停测试靶场未返回成功状态",
...
def preflight_element_library(
    ...
    try:
        ...
        hover_matches = _library_named_matches(payload, hover_element_name)
        copy_matches = _library_named_matches(payload, copy_element_name)
        hover_page_ok, hover_selector_ok = _library_item_ok(hover_matches, target_url)
        copy_page_ok, copy_selector_ok = _library_item_ok(copy_matches, target_url)
        passed = (
            library_dir.is_dir()
            and elements_file.is_file()
            and hover_page_ok
            and hover_selector_ok
            and copy_page_ok
        ...
        return _result(
            ...
            "PASS" if passed else "BLOCKED",
            "元素库包含hover与复制记录元素" if passed else "元素库缺少hover或复制记录元素",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
            library_dir_exists=library_dir.is_dir(),
            elements_json_exists=elements_file.is_file(),
            hover_named_element_count=len(hover_matches),
            copy_named_element_count=len(copy_matches),
            hover_page_url_matches=hover_page_ok,
            copy_page_url_matches=copy_page_ok,
            hover_selector_present=hover_selector_ok,
    ...
    try:
        ...
        return (
            _result(
                "owned_page_setup",
                "PASS" if passed else "BLOCKED",
                "已打开本次唯一悬停测试页面" if passed else "悬停测试页面创建结果不完整",
                elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
    ...
    try:
        ...
        return (
            _result(
                ...
                "本次 Chrome 页面窗口已处于可执行真实鼠标悬停的 normal 状态"
                if normalized
...
def connect_library_and_find_elements(
    ...
    try:
        ...
        hover_element = page.find(hover_element_name, timeout=element_timeout)
        copy_element = page.find(copy_element_name, timeout=element_timeout)
        hover_name_ok = str(hover_element.name or "") == hover_element_name
        copy_name_ok = str(copy_element.name or "") == copy_element_name
        passed = package.web_count > 0 and hover_name_ok and copy_name_ok
        return (
            _result(
                ...
                hover_element_name_ok=hover_name_ok,
...
def _copy_latest_record(copy_element: WebElement) -> tuple[dict[str, Any] | None, str]:
    sentinel = f"uiautoma_hover_sentinel_{uuid.uuid4().hex}"
...
def run_hover_case(
    ...
    min_elapsed_ms = float(case.get("min_elapsed_ms") or 0.0)
    simulative = bool(kwargs.get("simulative", True))
    try:
        page.activate()
        time.sleep(0.1)
        call_started = time.perf_counter()
        returned = hover_element.hover(**kwargs)
        ...
        keys_ok = str(fields["detected_keys"]) == EXPECTED_HOVER_KEYS
        event_ok = str(fields["event_type"]) == EXPECTED_HOVER_EVENT_TYPE
        ...
        return _result(
            case_id,
            "PASS" if passed else "FAIL",
            f"{detail_prefix} hover 与剪贴板记录符合预期"
            if passed
            else f"{detail_prefix} hover 返回值或剪贴板记录不符合预期",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
            hover_elapsed_ms=elapsed_ms,
            return_is_none=return_is_none,
            copy_mode=copy_mode,
            expected_button_id=expected_button_id,
            expected_source=expected_source,
            expected_event_type=EXPECTED_HOVER_EVENT_TYPE,
            expected_keys=EXPECTED_HOVER_KEYS,
        ...
        return _result(
            case_id,
            status,
            f"{detail_prefix} hover 被环境阻塞"
            if status == "BLOCKED"
            else f"{detail_prefix} hover 调用失败",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
            simulative=simulative,
            **_error_fields(exc),
        )
    ...
    return (
        {
            ...
            "hover_element_name": args.hover_element_name,
        ...
        if scrub_result["status"] == "PASS":
            ...
            if page_result["status"] == "PASS" and page is not None:
                ...
                if (
                    ...
                    and hover_element is not None
                    and copy_element is not None
                ):
                    for case in BACKGROUND_HOVER_CASES:
                        results.append(
                            run_hover_case(page, hover_element, copy_element, case)
                        )
                        if results[-1]["status"] != "PASS":
                            break
                    if results[-1]["status"] == "PASS":
                        ...
                        if window_result["status"] == "PASS":
                            for case in SIMULATIVE_HOVER_CASES:
                                results.append(
                                    run_hover_case(page, hover_element, copy_element, case)
                                )
                                if results[-1]["status"] != "PASS":
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="运行 uiautoma.web.WebElement.hover() 持久化真实浏览器测试。"
    )
    parser.add_argument(
        "--mode",
        default="chrome",
    ...
    parser.add_argument(
        ...
        help=f"悬停测试靶场，默认 {DEFAULT_TARGET_URL}",
    )
    parser.add_argument(
        "--element-library",
        type=Path,
        default=DEFAULT_ELEMENT_LIBRARY,
        help="包含悬停与复制元素的元素库目录。",
    )
    parser.add_argument(
        "--hover-element-name",
        default=DEFAULT_HOVER_ELEMENT_NAME,
        help=f"悬停目标元素名称，默认 {DEFAULT_HOVER_ELEMENT_NAME}。",
    )
    parser.add_argument(
        "--copy-element-name",
    ...
    args.hover_element_name = str(args.hover_element_name or "").strip()
    args.copy_element_name = str(args.copy_element_name or "").strip()
    args.profile_directory = str(args.profile_directory or "").strip()
    if not args.hover_element_name:
        parser.error("--hover-element-name 不能为空")
