"""uiautoma.web.WebElement.is_checked() 持久化真实浏览器测试运行器。

导入本模块不会连接 Runtime、访问靶场或操作浏览器。真实场景打开唯一 Chrome
``form-controls`` 页面并连接指定元素库；目标断言直接调用 ``WebElement.is_checked()``。

状态切换使用 ``click()``（场景准备，非目标 API）；``提交_html`` 将表单结果复制到
剪贴板，``重置_html`` 复位。HTML 目标使用库项 ``checkbox_html_input2``（绑定在
``input[type=checkbox]`` 上）。
"""
...
DEFAULT_HTML_INPUT_NAME = "checkbox_html_input2"
DEFAULT_ANT_INPUT_NAME = "checkbox_组件_input"
DEFAULT_SUBMIT_ELEMENT_NAME = "提交_html"
DEFAULT_RESET_ELEMENT_NAME = "重置_html"
RUN_QUERY_KEY = "uiautoma_element_is_checked_run"

EXPECTED_PARAMETER_ORDER = ("self",)
EXPECTED_DEFAULTS: dict[str, Any] = {}
EXPECTED_KINDS = {
    "self": inspect.Parameter.POSITIONAL_OR_KEYWORD,
}

# 目标 API：is_checked() -> bool
# 用户文案：选中 True，否则 False；实现读 checked 属性 true/checked/1。
    ...
    try:
        ...
        return (
            _result(
                ...
                "已打开本次唯一 is_checked 测试页面" if passed else "is_checked 测试页面创建结果不完整",
                elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
...
def check_contract() -> dict[str, Any]:
    try:
        signature = inspect.signature(WebElement.is_checked)
        ...
        return_ok = return_ann is bool or return_ann == "bool" or str(return_ann) in {"bool", "<class 'bool'>"}
...
def preflight_target(target_url: str, timeout: float) -> dict[str, Any]:
    ...
    try:
        ...
        return _result(
            "target_preflight",
            "PASS" if passed else "BLOCKED",
            "is_checked 测试靶场可访问" if passed else "is_checked 测试靶场未返回成功状态",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
...
def preflight_element_library(
    ...
    try:
        ...
        return _result(
            ...
            "元素库包含 is_checked 场景所需库项"
            if all_ok
            else "元素库缺少 is_checked 场景库项或 PageUrl/选择器不匹配",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
...
def _submit_form(submit_element: WebElement) -> dict[str, Any] | None:
    clipboard.set_text(f"uiautoma_is_checked_sentinel_{uuid.uuid4().hex}")
...
def run_is_checked_toggle_case(
    *,
    case_id: str,
    page: WebBrowser,
    target: WebElement,
    ...
    try:
        ...
        attr_unchecked = target.get_attribute("checked")
        print(
            f"{case_id}: after reset is_checked()={unchecked!r} attr.checked={attr_unchecked!r}",
            file=sys.stderr,
            flush=True,
        )
        unchecked_ok = unchecked is False

        target.click(simulative=False, delay_after=0.25)
        checked = target.is_checked()
        attr_checked = target.get_attribute("checked")
        print(
            f"{case_id}: after click is_checked()={checked!r} attr.checked={attr_checked!r}",
            file=sys.stderr,
            flush=True,
        )
        checked_ok = checked is True

        payload = _submit_form(submit_element)
        hobbies = None if payload is None else payload.get("hobbies")
        print(f"{case_id}: submit hobbies={hobbies!r}", file=sys.stderr, flush=True)
        if expect_hobbies_travel:
            form_ok = hobbies == ["旅行"] or hobbies == "旅行"
        else:
            form_ok = True

        _reset_form(reset_element)
        after_reset = target.is_checked()
        attr_after_reset = target.get_attribute("checked")
        print(
            f"{case_id}: after reset2 is_checked()={after_reset!r} attr.checked={attr_after_reset!r}",
            file=sys.stderr,
            flush=True,
        )
        reset_ok = after_reset is False

        type_ok = all(isinstance(v, bool) for v in (unchecked, checked, after_reset))
        passed = type_ok and unchecked_ok and checked_ok and reset_ok and form_ok
        return _result(
            case_id,
            "PASS" if passed else "FAIL",
            "is_checked 在未选/选中/复位三态与表单佐证符合预期"
            if passed
            else "is_checked 返回值或表单佐证不符合预期",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
            ...
            after_reset_value=after_reset,
            attr_checked_unchecked=attr_unchecked,
            attr_checked_after_click=attr_checked,
            attr_checked_after_reset=attr_after_reset,
            hobbies=hobbies,
        ...
        return _result(
            case_id,
            status,
            "is_checked 场景被环境阻塞" if status == "BLOCKED" else "is_checked 场景调用失败",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
            **_error_fields(exc),
        )
...
def _report(args: argparse.Namespace, results: Sequence[dict[str, Any]]) -> tuple[dict[str, Any], int]:
    ...
    excluded = [
        ...
        "cef",
        "auto",
        "以 check() 切换选中态（已知受控组件缺口，改用 click）",
        "radio / 非 checkbox 控件",
    ]
    return (
        {
            ...
            "status": status,
            "exit_code": exit_code,
            "lifecycle_hint": "READY_FOR_LIVE" if exit_code != 0 else "VERIFIED_CANDIDATE",
            "mode": args.mode,
            "profile_directory": args.profile_directory,
            "contract_only": bool(args.contract_only),
            "html_input_name": args.html_input_name,
            "ant_input_name": args.ant_input_name,
            "results": list(results),
            "excluded": excluded,
            "open_defects": [
                "is_checked 仅解析 get_attribute('checked') 字符串；勾选后属性常为空串，IDL checked 已为 true 时仍返回 False",
    ...
    try:
        ...
        if scrub_result["status"] == "PASS":
            ...
            if page_result["status"] == "PASS" and page is not None:
                ...
                html_input = found.get(args.html_input_name)
                ...
                if (
                    ...
                    and html_input is not None
                    and ant_input is not None
                    and submit_element is not None
                    and reset_element is not None
                ):
                    results.append(
                        run_is_checked_toggle_case(
                            case_id="html_input2_is_checked",
                            page=page,
                            target=html_input,
                            reset_element=reset_element,
                            submit_element=submit_element,
                            expect_hobbies_travel=True,
                        )
                    )
                    results.append(
                        run_is_checked_toggle_case(
                            case_id="ant_input_is_checked",
                            page=page,
                            target=ant_input,
                            reset_element=reset_element,
                            submit_element=submit_element,
                            expect_hobbies_travel=False,
                        )
                    )
    except Exception as exc:  # noqa: BLE001
        results.append(
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="运行 uiautoma.web.WebElement.is_checked() 持久化真实浏览器测试。"
    )
    parser.add_argument(
        "--mode",
        default="chrome",
    ...
    parser.add_argument(
        ...
        help=f"is_checked 测试靶场，默认 {DEFAULT_TARGET_URL}",
    )
    parser.add_argument(
        "--element-library",
    ...
    parser.add_argument("--html-input-name", default=DEFAULT_HTML_INPUT_NAME)
