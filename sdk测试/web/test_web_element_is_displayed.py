"""uiautoma.web.WebElement.is_displayed() 持久化真实浏览器测试运行器。

导入本模块不会连接 Runtime、访问靶场或操作浏览器。真实场景打开唯一 Chrome
``form-controls`` 页面并连接指定元素库；目标断言直接调用 ``WebElement.is_displayed()``。

用例（当前浏览器窗口大小；脚本不主动滚动页面）：
- ``input元素``：在视窗内，期望 ``True``
- ``提交_html``：在视窗外，期望 ``False``
"""
...
DEFAULT_IN_VIEWPORT_ELEMENT_NAME = "input元素"
DEFAULT_OUT_OF_VIEWPORT_ELEMENT_NAME = "提交_html"
RUN_QUERY_KEY = "uiautoma_element_is_displayed_run"
...
# 目标 API：is_displayed() -> bool
# 可见 True；不可见 False。公开语义为视窗可见性。
# 当前实现：exists(timeout=3) → locate；Page Engine locate 会 scrollIntoView。
    ...
    try:
        ...
        return (
            _result(
                ...
                "已打开本次唯一 is_displayed 测试页面" if passed else "is_displayed 测试页面创建结果不完整",
                elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
...
def check_contract() -> dict[str, Any]:
    try:
        signature = inspect.signature(WebElement.is_displayed)
...
def preflight_target(target_url: str, timeout: float) -> dict[str, Any]:
    ...
    try:
        ...
        return _result(
            "target_preflight",
            "PASS" if passed else "BLOCKED",
            "is_displayed 测试靶场可访问" if passed else "is_displayed 测试靶场未返回成功状态",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
...
def preflight_element_library(
    ...
    try:
        ...
        return _result(
            ...
            "元素库包含 is_displayed 场景所需库项"
            if all_ok
            else "元素库缺少 is_displayed 场景库项或 PageUrl/选择器不匹配",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
...
def _viewport_scroll_snapshot(page: WebBrowser) -> dict[str, Any]:
    """读取当前滚动位置；走页面脚本，不走元素 locate（避免二次 scrollIntoView）。"""
    try:
        result = page._runtime_client().web_browser_execute_javascript(  # noqa: SLF001
            "function () { return {"
            "scrollX: window.scrollX, scrollY: window.scrollY, "
            "innerWidth: window.innerWidth, innerHeight: window.innerHeight"
            "}; }",
            argument=None,
            **page._browser_params(timeout_ms=2000),  # noqa: SLF001
        )
        value = result.get("value") if isinstance(result, dict) else None
        return {"ok": True, "viewport": value if isinstance(value, dict) else result}
    except Exception as exc:  # noqa: BLE001
        return {"ok": False, **_error_fields(exc)}


def run_is_displayed_case(
    *,
    case_id: str,
    page: WebBrowser,
    target: WebElement,
    ...
    started = time.perf_counter()
    try:
        # 不主动滚动；调用前后只读 scrollY，用于验证 locate/校验路径是否 scrollIntoView。
        page.activate()
        time.sleep(0.1)
        before = _viewport_scroll_snapshot(page)
        call_started = time.perf_counter()
        value = target.is_displayed()
        call_elapsed_ms = round((time.perf_counter() - call_started) * 1000, 1)
        after = _viewport_scroll_snapshot(page)
        before_y = ((before.get("viewport") or {}) if before.get("ok") else {}).get("scrollY")
        after_y = ((after.get("viewport") or {}) if after.get("ok") else {}).get("scrollY")
        scrolled = (
            isinstance(before_y, (int, float))
            and isinstance(after_y, (int, float))
            and float(after_y) != float(before_y)
        )
        print(
            f"{case_id}: is_displayed()={value!r} expected={expected!r} "
            f"scrollY {before_y!r}->{after_y!r} scrolled={scrolled}",
            file=sys.stderr,
            flush=True,
        )
        type_ok = isinstance(value, bool)
        value_ok = value is expected
        passed = type_ok and value_ok
        return _result(
            case_id,
            "PASS" if passed else "FAIL",
            f"is_displayed 返回 {expected!r} 符合预期" if passed else f"is_displayed 期望 {expected!r} 实际 {value!r}",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
            is_displayed_elapsed_ms=call_elapsed_ms,
        ...
        return _result(
            case_id,
            status,
            "is_displayed 被环境阻塞" if status == "BLOCKED" else "is_displayed 调用失败",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
            expected=expected,
            **_error_fields(exc),
        )
...
def _report(args: argparse.Namespace, results: Sequence[dict[str, Any]]) -> tuple[dict[str, Any], int]:
    ...
    return (
        {
            "api": "uiautoma.web.WebElement.is_displayed",
            "status": status,
            "exit_code": exit_code,
            ...
            "in_viewport_element_name": args.in_viewport_element_name,
            "out_of_viewport_element_name": args.out_of_viewport_element_name,
            "results": list(results),
            "excluded": [
                "edge",
                "cef",
                "auto",
                "display:none / visibility:hidden / opacity:0 完整矩阵",
                "iframe / shadow 内元素可见性",
                "滚动到视窗后复读",
            ],
            "open_defects": [
                "is_displayed → exists → locate；Page Engine locateXPathResult 无条件 scrollIntoView(center)，"
                "视窗外元素会被滚入视窗且仍返回 True（未按可见性判定）",
                "https://github.com/uiautoma/desktop/issues/24",
            ],
        },
        exit_code,
    )
...
def run(args: argparse.Namespace) -> tuple[dict[str, Any], int]:
    names = [args.in_viewport_element_name, args.out_of_viewport_element_name]
    ...
    try:
        ...
        if scrub_result["status"] == "PASS":
            ...
            if page_result["status"] == "PASS" and page is not None:
                ...
                in_view_el = found.get(args.in_viewport_element_name)
                out_view_el = found.get(args.out_of_viewport_element_name)
                if library_result["status"] == "PASS" and in_view_el is not None and out_view_el is not None:
                    results.append(
                        run_is_displayed_case(
                            case_id="in_viewport_input_is_displayed_true",
                            page=page,
                            target=in_view_el,
                            expected=True,
                        )
                    ...
                    results.append(
                        run_is_displayed_case(
                            case_id="out_of_viewport_submit_is_displayed_false",
                            page=page,
                            target=out_view_el,
                            expected=False,
                        )
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="运行 uiautoma.web.WebElement.is_displayed() 持久化真实浏览器测试。"
    )
    ...
    parser.add_argument("--in-viewport-element-name", default=DEFAULT_IN_VIEWPORT_ELEMENT_NAME)
    parser.add_argument("--out-of-viewport-element-name", default=DEFAULT_OUT_OF_VIEWPORT_ELEMENT_NAME)
