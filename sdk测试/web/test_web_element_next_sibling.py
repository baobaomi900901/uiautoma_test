"""uiautoma.web.WebElement.next_sibling() 持久化真实浏览器测试运行器。

导入本模块不会连接 Runtime、访问靶场或操作浏览器。真实场景打开唯一 Chrome
页面并连接指定元素库；目标断言只对 ``九宫格_中`` 直接调用无参 ``WebElement.next_sibling()``，打印返回的
...
RUN_QUERY_KEY = "uiautoma_element_next_sibling_run"
...
# 与 children()/child_at() 实测顺序对齐：中心格的下一个兄弟为 div:右 right
EXPECTED_SIBLING_NAME_SUBSTR = '右 right'
...
def check_contract() -> dict[str, Any]:
    try:
        signature = inspect.signature(WebElement.next_sibling)
...
def run_next_sibling_case(page: WebBrowser, target: WebElement) -> dict[str, Any]:
    started = time.perf_counter()
    try:
        page.activate()
        time.sleep(0.1)
        ...
        return _result(
            "next_sibling_default",
            "PASS" if passed else "FAIL",
            "next_sibling 返回 WebElement 并已打印" if passed else "next_sibling 返回值不符合预期",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
            next_sibling_elapsed_ms=elapsed_ms,
        ...
        return _result(
            "next_sibling_default",
            status,
            "next_sibling 被环境阻塞" if status == "BLOCKED" else "next_sibling 调用失败",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
            **_error_fields(exc),
        )
        ...
        if scrub_result["status"] == "PASS":
            ...
            if page_result["status"] == "PASS" and page is not None:
                ...
                if library_result["status"] == "PASS" and target is not None:
                    results.append(run_next_sibling_case(page, target))
    except Exception as exc:  # noqa: BLE001
        results.append(
            _result(
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="运行 uiautoma.web.WebElement.next_sibling() 持久化真实浏览器测试。"
    )
    parser.add_argument(
        "--mode",
        default="chrome",
