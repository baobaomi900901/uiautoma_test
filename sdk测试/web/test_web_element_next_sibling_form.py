"""WebElement.next_sibling()：表单输入框及同级元素的独立验收。

预期结果由原生 nextElementSibling 提供；原有 test_web_element_next_sibling.py 保留。
...
DOM_ORACLE = """function (element, inputId) {
  ...
  for (let depth = 0; current && depth < 32; depth++) {
    ...
    ancestors.push({
      ...
      next: Array.from(current.children).map(node =>
        node.nextElementSibling ? info(node.nextElementSibling) : null),
  ...
  return {
    ...
    input_next: input.nextElementSibling ? info(input.nextElementSibling) : null,
...
def contract():
    sig = inspect.signature(WebElement.next_sibling)
    ...
    return result('api_contract', 'PASS' if ok else 'FAIL',
                  'timeout 仅限关键字、默认 5.0；返回下一个兄弟元素 WebElement，无对应元素返回 None'
...
def check_next(value, expected):
    if expected is None:
        if value is not None:
            raise AssertionError(f'预期没有下一个兄弟元素，返回 None；实际 {type(value).__name__}')
        return '原生 nextElementSibling=null；实际 None'
    actual = check_dom_element(value, expected)
    return f'预期 {summary(expected)}；实际 {actual}；outerHTML 与原生 nextElementSibling 一致'
...
def validate_dom(snapshot):
    ...
    for depth, row in enumerate(ancestors, 1):
        ...
        if row.get('next') != children[1:] + [None]:
            raise AssertionError(f'第 {depth} 层 nextElementSibling 与同级元素顺序不符')
    ...
    if 'input_next' not in snapshot or snapshot['input_next'] != ancestors[0]['next'][target_index]:
        raise AssertionError('输入框 nextElementSibling 参照与父容器不符')
...
def check_zero_timeout(element, expected):
    """仅零预算允许明确超时；正常返回必须是已知存在的下一个兄弟元素。"""
    try:
        value = element.next_sibling(timeout=0)
    except ActionError as exc:
        if exc.trace_info != 'web_dom_timeout':
            raise
        return (error_detail('零预算边界：预期正确兄弟元素或明确超时；实际超时', exc)
                + '；本次未取得兄弟元素，不按读取成功描述')
    return '零预算内返回兄弟元素；' + check_next(value, expected)


def run_next_sibling_cases(target, siblings, args, snapshot):
    results = []
    snapshot = validate_dom(snapshot)
    multi = snapshot['ancestors'][snapshot['multi_index']]
    expected = multi['next'][0]
    subject = siblings[0]
    record_case(results, 'next_default', lambda:
                check_next(subject.next_sibling(), expected))
    record_case(results, 'next_keyword', lambda:
                check_next(subject.next_sibling(timeout=args.timeout), expected))
    record_case(results, 'next_none_timeout', lambda:
                check_next(subject.next_sibling(timeout=None), expected))
    record_case(results, 'next_zero_timeout', lambda: check_zero_timeout(subject, expected))

    def repeat():
        for _ in range(3):
            detail = check_next(subject.next_sibling(timeout=args.timeout), expected)
        return '连续三次与原生下一个兄弟元素核对通过；' + detail
    record_case(results, 'repeat_next', repeat)

    def all_positions():
        for index, item in enumerate(siblings):
            try:
                check_next(item.next_sibling(timeout=args.timeout), multi['next'][index])
            except Exception as exc:
                raise AssertionError(error_detail(f'同级元素 index={index}', exc)) from exc
        return f'同级 {len(siblings)} 个元素逐项核对；末项为 None，其余为紧邻的下一个元素'
    record_case(results, 'all_sibling_positions', all_positions)
    record_case(results, 'last_none', lambda:
                check_next(siblings[-1].next_sibling(timeout=args.timeout), None))

    def repeat_none():
        for _ in range(2):
            check_next(siblings[-1].next_sibling(timeout=args.timeout), None)
        return '同级末项连续两次返回 None；未返回父元素或其后代'
    record_case(results, 'last_repeat', repeat_none)
    record_case(results, 'input_next', lambda:
                check_next(target.next_sibling(timeout=args.timeout), snapshot['input_next']))

    def forward_chain():
        current = siblings[0]
        for index in range(1, len(siblings)):
            current = current.next_sibling(timeout=args.timeout)
            check_next(current, multi['children'][index])
        check_next(current.next_sibling(timeout=args.timeout), None)
        return f'从首项向后逐个读取 {len(siblings) - 1} 个兄弟元素，再到 None；顺序与独立 DOM 一致'
    record_case(results, 'forward_chain', forward_chain)

    def returned_usable():
        following = subject.next_sibling(timeout=args.timeout)
        check_next(following, expected)
        check_dom_element(following.parent(timeout=args.timeout), multi['node'])
    ...
    record_case(results, 'negative_timeout', lambda:
                expect_exception(lambda: subject.next_sibling(timeout=-1), InvalidParamsError))
    record_case(results, 'negative_fraction', lambda:
                expect_exception(lambda: subject.next_sibling(timeout=-0.1), InvalidParamsError))
    record_case(results, 'nonnumeric_timeout', lambda:
                expect_exception(lambda: subject.next_sibling(timeout='bad'), InvalidParamsError))
    record_case(results, 'positional_timeout', lambda:
                expect_exception(lambda: subject.next_sibling(1), TypeError))
    record_case(results, 'unknown_keyword', lambda:
                expect_exception(lambda: subject.next_sibling(unsupported=True), TypeError))
...
def cleanup(resources, results, temp_root: Path):
    ...
    if temp_dir is not None:
        def remove_copy():
            ...
            if resolved.parent != temp_root.resolve() or not resolved.name.startswith('uiautoma-element-next-sibling-'):
...
def run(args):
    ...
    try:
        ...
        temp_dir = Path(tempfile.mkdtemp(prefix='uiautoma-element-next-sibling-', dir=args.temp_root))
        ...
        prepared('用 parent() 取得容器并与原生祖先逐层核对；未用 next_sibling() 生成期望值')

        stage, started = 'siblings_prepare', time.perf_counter()
        siblings = prepare_siblings(multi, snapshot, timeout=args.timeout)
        prepared(f'用 child_at() 取得同级 {len(siblings)} 个元素并与原生 DOM 核对；期望值来自 nextElementSibling')

        stage, started = 'next_sibling_cases', time.perf_counter()
        results.extend(run_next_sibling_cases(target, siblings, args, snapshot))
    except Exception as exc:
...
def main(argv=None):
    parser = argparse.ArgumentParser(description='WebElement.next_sibling() 表单兄弟元素独立验收')
    ...
    print('API     : uiautoma.web.WebElement.next_sibling', flush=True)
    ...
    print('参照    : 原生 nextElementSibling 及 outerHTML；没有下一个兄弟元素返回 None', flush=True)
    started = time.perf_counter()
