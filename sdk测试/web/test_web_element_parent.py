"""uiautoma.web.WebElement.parent() 持久化真实浏览器测试运行器。

导入本模块不会连接 Runtime、访问靶场或操作浏览器。真实场景打开唯一 Chrome
页面并连接指定元素库；目标断言只直接调用无参 ``WebElement.parent()``，打印返回的
父元素，并确认其为 ``WebElement``。
"""
...
DEFAULT_PARENT_ELEMENT_NAME = "九宫格_中"
DEFAULT_PROFILE_DIRECTORY = "Default"
RUN_QUERY_KEY = "uiautoma_element_parent_run"
    ...
    try:
        ...
        return (
            _result(
                ...
                "已打开本次唯一父元素测试页面" if passed else "父元素测试页面创建结果不完整",
                elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
...
def check_contract() -> dict[str, Any]:
    try:
        signature = inspect.signature(WebElement.parent)
...
def preflight_target(target_url: str, timeout: float) -> dict[str, Any]:
    ...
    try:
        ...
        return _result(
            "target_preflight",
            "PASS" if passed else "BLOCKED",
            "父元素测试靶场可访问" if passed else "父元素测试靶场未返回成功状态",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
...
def preflight_element_library(
    ...
    try:
        ...
        payload = json.loads(elements_file.read_text(encoding="utf-8"))
        parent_matches = _library_named_matches(payload, parent_element_name)
        parent_page_ok, parent_selector_ok = _library_item_ok(parent_matches, target_url)
        passed = (
            library_dir.is_dir()
            and elements_file.is_file()
            and parent_page_ok
            and parent_selector_ok
        )
        return _result(
            "element_library_preflight",
            "PASS" if passed else "BLOCKED",
            "元素库包含 九宫格_中" if passed else "元素库缺少 九宫格_中",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
            library_dir_exists=library_dir.is_dir(),
            elements_json_exists=elements_file.is_file(),
            parent_named_element_count=len(parent_matches),
            parent_page_url_matches=parent_page_ok,
            parent_selector_present=parent_selector_ok,
        )
    except Exception as exc:  # noqa: BLE001
        return _result(
            "element_library_preflight",
...
def connect_library_and_find_element(
    ...
    try:
        ...
        target = page.find(parent_element_name, timeout=element_timeout)
        name_ok = str(target.name or "") == parent_element_name
        passed = package.web_count > 0 and name_ok
        return (
            _result(
                "element_library_setup",
                "PASS" if passed else "FAIL",
                "已连接元素库并绑定 九宫格_中" if passed else "元素库连接或元素绑定结果不符合预期",
                elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
                package_opened=True,
                web_element_count=package.web_count,
                parent_element_name_ok=name_ok,
            ),
            package,
            target,
        )
...
def _parent_public_fields(element: WebElement | None) -> dict[str, Any]:
    if element is None:
        return {
            "parent_repr": "",
            "parent_id": "",
    ...
    return {
        "parent_repr": str(element),
        "parent_id": str(element.id or ""),
        "parent_name": str(element.name or ""),
        "parent_xpath": xpath,
        "parent_raw_keys": sorted(str(key) for key in raw.keys()),
    }


def run_parent_case(page: WebBrowser, target: WebElement) -> dict[str, Any]:
    started = time.perf_counter()
    try:
        page.activate()
        time.sleep(0.1)
        ...
        print(parent, file=sys.stderr, flush=True)
        fields = _parent_public_fields(parent)
        type_ok = isinstance(parent, WebElement)
        identity_ok = bool(fields["parent_id"] or fields["parent_xpath"] or fields["parent_name"])
        different_ok = str(parent.id or "") != str(target.id or "") or fields["parent_xpath"] != str(
            (target.raw or {}).get("xpath") or (target.raw or {}).get("Characteristics") or ""
        )
        passed = type_ok and identity_ok and different_ok
        return _result(
            "parent_default",
            "PASS" if passed else "FAIL",
            "parent 返回 WebElement 并已打印" if passed else "parent 返回值不符合预期",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
            parent_elapsed_ms=elapsed_ms,
            return_type_ok=type_ok,
            parent_identity_ok=identity_ok,
            parent_different_from_target_ok=different_ok,
        ...
        return _result(
            "parent_default",
            status,
            "parent 被环境阻塞" if status == "BLOCKED" else "parent 调用失败",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
            **_error_fields(exc),
        )
    ...
    return (
        {
            ...
            "parent_element_name": args.parent_element_name,
        ...
        if scrub_result["status"] == "PASS":
            ...
            if page_result["status"] == "PASS" and page is not None:
                ...
                if library_result["status"] == "PASS" and target is not None:
                    results.append(run_parent_case(page, target))
    except Exception as exc:  # noqa: BLE001
        results.append(
            _result(
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="运行 uiautoma.web.WebElement.parent() 持久化真实浏览器测试。"
    )
    parser.add_argument(
        "--mode",
        default="chrome",
    ...
    parser.add_argument(
        ...
        help=f"父元素测试靶场，默认 {DEFAULT_TARGET_URL}",
    )
    parser.add_argument(
        "--element-library",
        type=Path,
    ...
    parser.add_argument(
        "--parent-element-name",
        default=DEFAULT_PARENT_ELEMENT_NAME,
        help=f"目标元素名称，默认 {DEFAULT_PARENT_ELEMENT_NAME}。",
    )
    ...
    args.parent_element_name = str(args.parent_element_name or "").strip()
    args.profile_directory = str(args.profile_directory or "").strip()
    if not args.parent_element_name:
        parser.error("--parent-element-name 不能为空")
