"""uiautoma.web.WebElement.previous_sibling() 持久化真实浏览器测试运行器。

导入本模块不会连接 Runtime、访问靶场或操作浏览器。真实场景打开唯一 Chrome
页面并连接指定元素库；目标断言只对 ``九宫格_中`` 直接调用无参 ``WebElement.previous_sibling()``，打印返回的
兄弟元素，并确认其为 ``WebElement``。
"""
...
RUN_QUERY_KEY = "uiautoma_element_previous_sibling_run"
...
# 与 children()/child_at() 实测顺序对齐：中心格的上一个兄弟为 div:左 left
EXPECTED_SIBLING_NAME_SUBSTR = '左 left'
...
def check_contract() -> dict[str, Any]:
    try:
        signature = inspect.signature(WebElement.previous_sibling)
...
def preflight_target(target_url: str, timeout: float) -> dict[str, Any]:
    ...
    try:
        ...
        return _result(
            "target_preflight",
            "PASS" if passed else "BLOCKED",
            "兄弟元素测试靶场可访问" if passed else "兄弟元素测试靶场未返回成功状态",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
...
def _sibling_public_fields(element: WebElement | None) -> dict[str, Any]:
    if element is None:
        return {
            "sibling_repr": "",
            "sibling_id": "",
    ...
    return {
        "sibling_repr": str(element),
        "sibling_id": str(element.id or ""),
        "sibling_name": str(element.name or ""),
        "sibling_xpath": xpath,
        "sibling_raw_keys": sorted(str(key) for key in raw.keys()),
    }


def run_previous_sibling_case(page: WebBrowser, target: WebElement) -> dict[str, Any]:
    started = time.perf_counter()
    try:
        page.activate()
        time.sleep(0.1)
        call_started = time.perf_counter()
        sibling = target.previous_sibling()
        elapsed_ms = round((time.perf_counter() - call_started) * 1000, 1)
        print(sibling, file=sys.stderr, flush=True)
        fields = _sibling_public_fields(sibling)
        type_ok = isinstance(sibling, WebElement)
        identity_ok = bool(fields["sibling_id"] or fields["sibling_xpath"] or fields["sibling_name"])
        different_ok = str(sibling.id or "") != str(target.id or "") or fields["sibling_xpath"] != str(
            (target.raw or {}).get("xpath") or (target.raw or {}).get("Characteristics") or ""
        )
        expected_name_substr = EXPECTED_SIBLING_NAME_SUBSTR
        name_substr_ok = expected_name_substr.casefold() in str(fields["sibling_name"] or "").casefold()
        passed = type_ok and identity_ok and different_ok and name_substr_ok
        return _result(
            "previous_sibling_default",
            "PASS" if passed else "FAIL",
            "previous_sibling 返回 WebElement 并已打印" if passed else "previous_sibling 返回值不符合预期",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
            previous_sibling_elapsed_ms=elapsed_ms,
            return_type_ok=type_ok,
            sibling_identity_ok=identity_ok,
            sibling_different_from_target_ok=different_ok,
            expected_name_substr=expected_name_substr,
            name_substr_ok=name_substr_ok,
        ...
        return _result(
            "previous_sibling_default",
            status,
            "previous_sibling 被环境阻塞" if status == "BLOCKED" else "previous_sibling 调用失败",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
            **_error_fields(exc),
        )
...
def _report(args: argparse.Namespace, results: Sequence[dict[str, Any]]) -> tuple[dict[str, Any], int]:
    ...
    excluded = [
        ...
        "auto",
        "显式传入 timeout 非默认值",
        "兄弟元素与库项名称一一对应；首尾格无兄弟负例",
        "元素库捕获期 WebSessionId 在不清理时仍可直连当前页面（产品侧会话绑定）",
    ]
    return (
        {
            "api": "uiautoma.web.WebElement.previous_sibling",
            "status": status,
            "exit_code": exit_code,
            "mode": args.mode,
        ...
        if scrub_result["status"] == "PASS":
            ...
            if page_result["status"] == "PASS" and page is not None:
                ...
                if library_result["status"] == "PASS" and target is not None:
                    results.append(run_previous_sibling_case(page, target))
    except Exception as exc:  # noqa: BLE001
        results.append(
            _result(
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="运行 uiautoma.web.WebElement.previous_sibling() 持久化真实浏览器测试。"
    )
    parser.add_argument(
        "--mode",
        default="chrome",
    ...
    parser.add_argument(
        ...
        help=f"兄弟元素测试靶场，默认 {DEFAULT_TARGET_URL}",
    )
    parser.add_argument(
        "--element-library",
        type=Path,
