"""WebElement.previous_sibling()：表单输入框及同级元素的独立验收。

预期结果由原生 previousElementSibling 提供；原有 test_web_element_previous_sibling.py 保留。
本文件只依赖标准库和 uiautoma，不依赖其它测试脚本。
源码快照：D:/code/desktop @ c101caa9dcd115a461fc71ecaed351b0dea880b8。
状态：VERIFIED（2026-09-19 用户实测 28/28；含预期的零预算超时边界）。
"""
...
DOM_ORACLE = """function (element, inputId) {
  ...
  for (let depth = 0; current && depth < 32; depth++) {
    ...
    ancestors.push({
      ...
      previous: Array.from(current.children).map(node =>
        node.previousElementSibling ? info(node.previousElementSibling) : null),
  ...
  return {
    ...
    input_previous: input.previousElementSibling ? info(input.previousElementSibling) : null,
...
def contract():
    sig = inspect.signature(WebElement.previous_sibling)
    ...
    return result('api_contract', 'PASS' if ok else 'FAIL',
                  'timeout 仅限关键字、默认 5.0；返回上一个兄弟元素 WebElement，无对应元素返回 None'
...
def check_previous(value, expected):
    if expected is None:
        if value is not None:
            raise AssertionError(f'预期没有前一个兄弟元素，返回 None；实际 {type(value).__name__}')
        return '原生 previousElementSibling=null；实际 None'
    actual = check_dom_element(value, expected)
    return f'预期 {summary(expected)}；实际 {actual}；outerHTML 与原生 previousElementSibling 一致'
...
def validate_dom(snapshot):
    ...
    for depth, row in enumerate(ancestors, 1):
        ...
        if row.get('previous') != [None] + children[:-1]:
            raise AssertionError(f'第 {depth} 层 previousElementSibling 与同级元素顺序不符')
    ...
    if 'input_previous' not in snapshot or snapshot['input_previous'] != ancestors[0]['previous'][target_index]:
        raise AssertionError('输入框 previousElementSibling 参照与父容器不符')
...
def prepare_siblings(container, snapshot, *, timeout):
    expected = snapshot['ancestors'][snapshot['multi_index']]['children']
    siblings = []
    for index, reference in enumerate(expected):
        value = container.child_at(index, timeout=timeout)
        try:
            check_dom_element(value, reference)
        except Exception as exc:
            raise AssertionError(error_detail(f'同级元素 index={index} 准备失败', exc)) from exc
        siblings.append(value)
    return siblings


def check_zero_timeout(element, expected):
    """仅零预算允许明确超时；正常返回必须是已知存在的前一个兄弟元素。"""
    try:
        value = element.previous_sibling(timeout=0)
    except ActionError as exc:
        if exc.trace_info != 'web_dom_timeout':
            raise
        return (error_detail('零预算边界：预期正确兄弟元素或明确超时；实际超时', exc)
                + '；本次未取得兄弟元素，不按读取成功描述')
    return '零预算内返回兄弟元素；' + check_previous(value, expected)


def run_previous_sibling_cases(target, siblings, args, snapshot):
    results = []
    snapshot = validate_dom(snapshot)
    multi = snapshot['ancestors'][snapshot['multi_index']]
    expected = multi['previous'][1]
    subject = siblings[1]
    record_case(results, 'previous_default', lambda:
                check_previous(subject.previous_sibling(), expected))
    record_case(results, 'previous_keyword', lambda:
                check_previous(subject.previous_sibling(timeout=args.timeout), expected))
    record_case(results, 'previous_none_timeout', lambda:
                check_previous(subject.previous_sibling(timeout=None), expected))
    record_case(results, 'previous_zero_timeout', lambda: check_zero_timeout(subject, expected))

    def repeat():
        for _ in range(3):
            detail = check_previous(subject.previous_sibling(timeout=args.timeout), expected)
        return '连续三次与原生前一个兄弟元素核对通过；' + detail
    record_case(results, 'repeat_previous', repeat)

    def all_positions():
        for index, item in enumerate(siblings):
            try:
                check_previous(item.previous_sibling(timeout=args.timeout), multi['previous'][index])
            except Exception as exc:
                raise AssertionError(error_detail(f'同级元素 index={index}', exc)) from exc
        return f'同级 {len(siblings)} 个元素逐项核对；首项为 None，其余为紧邻的前一个元素'
    record_case(results, 'all_sibling_positions', all_positions)
    record_case(results, 'first_none', lambda:
                check_previous(siblings[0].previous_sibling(timeout=args.timeout), None))

    def repeat_none():
        for _ in range(2):
            check_previous(siblings[0].previous_sibling(timeout=args.timeout), None)
        return '同级首项连续两次返回 None；未返回父元素或其后代'
    record_case(results, 'first_repeat', repeat_none)
    record_case(results, 'input_previous', lambda:
                check_previous(target.previous_sibling(timeout=args.timeout), snapshot['input_previous']))

    def backward_chain():
        current = siblings[-1]
        for index in range(len(siblings) - 2, -1, -1):
            current = current.previous_sibling(timeout=args.timeout)
            check_previous(current, multi['children'][index])
        check_previous(current.previous_sibling(timeout=args.timeout), None)
        return f'从末项向前逐个读取 {len(siblings) - 1} 个兄弟元素，再到 None；顺序与独立 DOM 一致'
    record_case(results, 'backward_chain', backward_chain)

    def returned_usable():
        previous = subject.previous_sibling(timeout=args.timeout)
        check_previous(previous, expected)
        check_dom_element(previous.parent(timeout=args.timeout), multi['node'])
        return '返回的兄弟元素可读取 outerHTML 和 parent()；仍属于原生共同父容器'
    record_case(results, 'returned_sibling_usable', returned_usable)
    record_case(results, 'negative_timeout', lambda:
                expect_exception(lambda: subject.previous_sibling(timeout=-1), InvalidParamsError))
    record_case(results, 'negative_fraction', lambda:
                expect_exception(lambda: subject.previous_sibling(timeout=-0.1), InvalidParamsError))
    record_case(results, 'nonnumeric_timeout', lambda:
                expect_exception(lambda: subject.previous_sibling(timeout='bad'), InvalidParamsError))
    record_case(results, 'positional_timeout', lambda:
                expect_exception(lambda: subject.previous_sibling(1), TypeError))
    record_case(results, 'unknown_keyword', lambda:
                expect_exception(lambda: subject.previous_sibling(unsupported=True), TypeError))
...
def cleanup(resources, results, temp_root: Path):
    ...
    if temp_dir is not None:
        def remove_copy():
            ...
            if resolved.parent != temp_root.resolve() or not resolved.name.startswith('uiautoma-element-previous-sibling-'):
...
def run(args):
    ...
    try:
        ...
        temp_dir = Path(tempfile.mkdtemp(prefix='uiautoma-element-previous-sibling-', dir=args.temp_root))
        ...
        prepared('用 parent() 取得容器并与原生祖先逐层核对；未用 previous_sibling() 生成期望值')

        stage, started = 'siblings_prepare', time.perf_counter()
        siblings = prepare_siblings(multi, snapshot, timeout=args.timeout)
        prepared(f'用 child_at() 取得同级 {len(siblings)} 个元素并与原生 DOM 核对；期望值来自 previousElementSibling')

        stage, started = 'previous_sibling_cases', time.perf_counter()
        results.extend(run_previous_sibling_cases(target, siblings, args, snapshot))
    except Exception as exc:
...
def main(argv=None):
    parser = argparse.ArgumentParser(description='WebElement.previous_sibling() 表单兄弟元素独立验收')
    ...
    print('API     : uiautoma.web.WebElement.previous_sibling', flush=True)
    ...
    print('参照    : 原生 previousElementSibling 及 outerHTML；没有前一个兄弟元素返回 None', flush=True)
    started = time.perf_counter()
