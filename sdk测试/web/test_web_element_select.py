"""WebElement.select() 实测：原生单选 select 与 Ant 自定义下拉框边界。
...
def check_contract(WebElement: type) -> dict[str, Any]:
    ...
    try:
        signature = inspect.signature(WebElement.select)
        parameters = list(signature.parameters.values())
        ok = (tuple(signature.parameters) == ("self", "item", "mode", "delay_after")
              ...
              and str(parameters[1].annotation) in {"str", "<class 'str'>"}
        ...
        return row("api_contract", "PASS" if ok else "FAIL",
                   "select(self, item: str, *, mode='fuzzy', delay_after=1) -> None",
...
def native_case(page: Any, case_id: str, item: str, mode: str, expected_value: str,
                expected_city: str | None, *, use_defaults: bool = False,
                delay: float = 0.05, preselect: bool = False) -> dict[str, Any]:
    started = time.perf_counter()
    expected = (f"{'先选择北京；' if preselect else ''}select({item!r}, mode={mode!r}) 返回 None；DOM value={expected_value!r}；"
                f"提交 city={expected_city!r}；重置 value=''" )
    try:
        ...
        if preselect:
            target.select("北京", mode="exact", delay_after=0.2)
            if state(page)["native"]["value"] != "beijing":
                raise RuntimeError("无匹配用例的北京前置选择未建立")
        invoked = time.perf_counter()
        returned = target.select(item) if use_defaults else target.select(item, mode=mode, delay_after=delay)
        ...
        while True:
            ...
            if current == expected_value or time.monotonic() >= deadline:
                break
            time.sleep(0.1)
        payload = submit_native(page, expected_city)
        after_reset = reset_native(page)
        passed = (returned is None and current == expected_value and payload.get("city") == expected_city
                  and after_reset == "" and (not use_defaults or call_ms >= 850))
...
def invalid_case(page: Any, case_id: str, invocation: str, expected_type: type,
                 message_fragment: str) -> dict[str, Any]:
    started = time.perf_counter()
    expected = f"{invocation} 抛出 {expected_type.__name__} 且含 {message_fragment!r}；DOM value 不变"
    try:
        before = reset_native(page)
        target = page.find(NATIVE, timeout=8)
        try:
            if case_id == "mode_value_removed":
                returned = target.select("北京", mode="value", delay_after=0)
            elif case_id == "regex_invalid":
                returned = target.select("[", mode="regex", delay_after=0)
            elif case_id == "item_missing":
                returned = target.select()
            elif case_id == "mode_positional":
                returned = target.select("北京", "exact")
            else:
...
def ant_boundary(page: Any, ActionError: type) -> dict[str, Any]:
    started = time.perf_counter()
    expected = "Ant 库元素为 input[role=combobox]；select('北京', exact) 拒绝且无状态变化"
    try:
        ...
        try:
            returned = target.select("北京", mode="exact", delay_after=0)
...
def run(args: argparse.Namespace) -> tuple[list[dict[str, Any]], int, str]:
    ...
    temp_dir = args.test_root / ".pytest_tmp" / "select" / uuid.uuid4().hex
    package = page = None
    native_ref = None
    baseline = None
    ...
    try:
        ...
        rows.append(native_case(page, "fuzzy_default", "京", "fuzzy", "beijing", "beijing", use_defaults=True))
        rows.append(native_case(page, "exact_shanghai", "上海", "exact", "shanghai", "shanghai"))
        rows.append(native_case(page, "regex_guangzhou", "^广", "regex", "guangzhou", "guangzhou"))
        rows.append(native_case(page, "exact_no_match", "不存在城市", "exact", "beijing", "beijing",
                                preselect=True))
        rows.append(native_case(page, "exact_case_sensitive", "beijing", "exact", "", ""))
        rows.append(invalid_case(page, "mode_value_removed", "mode='value'", InvalidParamsError, "mode"))
        rows.append(invalid_case(page, "regex_invalid", "mode='regex', item='['", InvalidParamsError, "正则"))
        rows.append(invalid_case(page, "item_missing", "缺少 item", TypeError, "item"))
        rows.append(invalid_case(page, "mode_positional", "mode 以位置传入", TypeError, "positional"))
    except Exception as exc:
        ...
        if native_ref is not None and package is not None:
            ...
            try:
                returned = native_ref.select("北京", mode="exact", delay_after=0)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="WebElement.select() iframe/Shadow 双侧表单实测")
    ...
    report = {"api": "uiautoma.web.WebElement.select", "url": args.target_url,
