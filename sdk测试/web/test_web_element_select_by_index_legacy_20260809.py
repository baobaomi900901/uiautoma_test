"""uiautoma.web.WebElement.select_by_index() 持久化真实浏览器测试运行器。

导入本模块不会连接 Runtime、访问靶场或操作浏览器。真实场景打开唯一 Chrome
``form-controls`` 页面并连接指定元素库；对 ``select_html`` 直接调用
``WebElement.select_by_index(...)``，再用 ``get_value()`` 与提交剪贴板 ``city`` 核对。
"""
...
RUN_QUERY_KEY = "uiautoma_element_select_by_index_run"
...
EXPECTED_KINDS = {
    ...
    "index": inspect.Parameter.POSITIONAL_OR_KEYWORD,
    "delay_after": inspect.Parameter.POSITIONAL_OR_KEYWORD,
}

# get_value()/提交剪贴板仅为验收；目标 API 是 select_by_index(index, delay_after=...)。
# select_html 选项：0 占位空值，1=beijing，2=shanghai，3=guangzhou，4=shenzhen。
    ...
    try:
        ...
        return (
            _result(
                ...
                "已打开本次唯一 select_by_index 测试页面"
                if passed
                else "select_by_index 测试页面创建结果不完整",
                elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
...
def check_contract() -> dict[str, Any]:
    try:
        ...
        index_required = (
            "index" in parameters
            and parameters["index"].default is inspect.Parameter.empty
        )
        ...
        return _result(
            ...
            index_required_ok=index_required,
...
def preflight_target(target_url: str, timeout: float) -> dict[str, Any]:
    ...
    try:
        ...
        return _result(
            "target_preflight",
            "PASS" if passed else "BLOCKED",
            "select_by_index 测试靶场可访问" if passed else "select_by_index 测试靶场未返回成功状态",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
...
def preflight_element_library(
    ...
    try:
        ...
        return _result(
            ...
            "PASS" if passed else "BLOCKED",
            "元素库包含 select_html、重置_html 与 提交_html"
            if passed
            else "元素库缺少 select_html、重置_html 或 提交_html",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
...
def connect_library_and_find_elements(
    ...
    try:
        ...
        passed = package.web_count > 0 and select_name_ok and reset_name_ok and submit_name_ok
        return (
            _result(
                "element_library_setup",
                "PASS" if passed else "FAIL",
                "已连接元素库并绑定 select_html、重置_html 与 提交_html"
                if passed
...
def _submit_city(submit_element: WebElement) -> object:
    clipboard.set_text(f"uiautoma_select_by_index_sentinel_{uuid.uuid4().hex}")
    submit_element.click(simulative=False, delay_after=0.35)
    try:
        payload = json.loads(clipboard.get_text())
    except Exception:  # noqa: BLE001
        return None
    return payload.get("city") if isinstance(payload, dict) else None


def run_select_by_index_case(
    page: WebBrowser,
    ...
    *,
    ...
    try:
        ...
        ret = select_element.select_by_index(index, delay_after=delay_after)
        select_elapsed_ms = round((time.perf_counter() - call_started) * 1000, 1)
        value = select_element.get_value()
        city = _submit_city(submit_element)
        print(
            f"select_by_index({index}, delay_after={delay_after}) -> {ret!r}; "
            f"value={value!r}; city={city!r}",
            file=sys.stderr,
            flush=True,
        )
        return_none_ok = ret is None
        value_ok = str(value) == expected_value
        # 占位空值时提交字段可能是 "" 或缺失；统一按字符串比较。
        city_normalized = "" if city is None else str(city)
        city_ok = city_normalized == expected_value
        delay_ok = (not require_delay) or select_elapsed_ms >= 900
        passed = return_none_ok and value_ok and city_ok and delay_ok
        return _result(
            case_id,
            "PASS" if passed else "FAIL",
            "select_by_index 后 value/city 符合预期" if passed else "select_by_index 验收不符合预期",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
            select_by_index_elapsed_ms=select_elapsed_ms,
        ...
        return _result(
            case_id,
            status,
            "select_by_index 被环境阻塞" if status == "BLOCKED" else "select_by_index 调用失败",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
            index=index,
            expected_value=expected_value,
            **_error_fields(exc),
    ...
    try:
        ...
        if scrub_result["status"] == "PASS":
            ...
            if page_result["status"] == "PASS" and page is not None:
                library_result, package, select_element, reset_element, submit_element = (
                ...
                if (
                    ...
                    results.append(
                        run_select_by_index_case(
                            page,
                            select_element,
                            reset_element,
                            submit_element,
                            case_id="select_by_index_beijing",
                            index=1,
                            expected_value="beijing",
                        )
                    )
                    results.append(
                        run_select_by_index_case(
                            ...
                            reset_element,
                            submit_element,
                            case_id="select_by_index_shanghai",
                            index=2,
                            expected_value="shanghai",
                        )
                    )
                    results.append(
                        run_select_by_index_case(
                            ...
                            reset_element,
                            submit_element,
                            case_id="select_by_index_placeholder",
                            index=0,
                            expected_value="",
                        )
                    )
                    results.append(
                        run_select_by_index_case(
                            ...
                            reset_element,
                            submit_element,
                            case_id="select_by_index_delay_after",
                            index=2,
                            expected_value="shanghai",
                            delay_after=1,
                            require_delay=True,
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="运行 uiautoma.web.WebElement.select_by_index() 持久化真实浏览器测试。"
    )
    parser.add_argument(
        "--mode",
        default="chrome",
    ...
    parser.add_argument(
        ...
        help=f"select_by_index 测试靶场，默认 {DEFAULT_TARGET_URL}",
    )
    parser.add_argument(
        "--element-library",
        type=Path,
        default=DEFAULT_ELEMENT_LIBRARY,
        help="包含 select_html、重置_html 与 提交_html 的元素库目录。",
    )
    parser.add_argument(
        "--select-element-name",
