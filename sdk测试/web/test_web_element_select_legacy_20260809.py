"""uiautoma.web.WebElement.select() 2026-08-09 旧靶场测试脚本。

导入本模块不会连接 Runtime、访问靶场或操作浏览器。真实场景打开唯一 Chrome
``form-controls`` 页面并连接指定元素库；对 ``select_html`` 直接调用
``WebElement.select(...)``，再用 ``get_value()`` 与提交剪贴板 ``city`` 核对。
"""
...
RUN_QUERY_KEY = "uiautoma_element_select_run"
...
# get_value()/提交剪贴板仅为验收；目标 API 是 select(item, mode=..., delay_after=...)。
# 公开 mode 枚举为 fuzzy/exact/value（不是 regex）。
    ...
    try:
        ...
        return (
            _result(
                ...
                "已打开本次唯一select 测试页面" if passed else "select 测试页面创建结果不完整",
                elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
...
def check_contract() -> dict[str, Any]:
    try:
        ...
        item_required = (
            "item" in parameters
            and parameters["item"].default is inspect.Parameter.empty
        )
        ...
        return _result(
            ...
            item_required_ok=item_required,
...
def preflight_target(target_url: str, timeout: float) -> dict[str, Any]:
    ...
    try:
        ...
        return _result(
            "target_preflight",
            "PASS" if passed else "BLOCKED",
            "select 测试靶场可访问" if passed else "select 测试靶场未返回成功状态",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
...
def _submit_city(submit_element: WebElement) -> object:
    clipboard.set_text(f"uiautoma_select_sentinel_{uuid.uuid4().hex}")
...
def run_select_case(
    ...
    *,
    ...
    try:
        ...
        ret = select_element.select(item, mode=mode, delay_after=delay_after)
        ...
        print(
            f"select({item!r}, mode={mode!r}, delay_after={delay_after}) -> {ret!r}; "
        ...
        city_ok = city == expected_value
        delay_ok = (not require_delay) or select_elapsed_ms >= 900
        passed = return_none_ok and value_ok and city_ok and delay_ok
        return _result(
            case_id,
            "PASS" if passed else "FAIL",
            "select 后 value/city 符合预期" if passed else "select 验收不符合预期",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
            select_elapsed_ms=select_elapsed_ms,
        ...
        return _result(
            case_id,
            status,
            "select 被环境阻塞" if status == "BLOCKED" else "select 调用失败",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
            item=item,
            mode=mode,
            expected_value=expected_value,
                ...
                if (
                    ...
                    results.append(
                        run_select_case(
                            ...
                            reset_element,
                            submit_element,
                            case_id="select_fuzzy_delay_after",
                            item="上海",
                            mode="fuzzy",
                            expected_value="shanghai",
                            delay_after=1,
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="运行 uiautoma.web.WebElement.select() 持久化真实浏览器测试。"
    )
    parser.add_argument(
        "--mode",
        default="chrome",
    ...
    parser.add_argument(
        ...
        help=f"select 测试靶场，默认 {DEFAULT_TARGET_URL}",
    )
    parser.add_argument(
        "--element-library",
        type=Path,
