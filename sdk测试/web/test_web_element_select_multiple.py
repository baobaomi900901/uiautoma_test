"""uiautoma.web.WebElement.select_multiple() 持久化真实浏览器测试运行器。

导入本模块不会连接 Runtime、访问靶场或操作浏览器。真实场景打开唯一 Chrome
``form-controls`` 页面并连接指定元素库；对 ``select_html_多选`` 直接调用
``WebElement.select_multiple(...)``。当前版本显式暂不支持该 API。
"""
...
RUN_QUERY_KEY = "uiautoma_element_select_multiple_run"
...
EXPECTED_KINDS = {
    ...
    "items": inspect.Parameter.POSITIONAL_OR_KEYWORD,
    "mode": inspect.Parameter.KEYWORD_ONLY,
    "append": inspect.Parameter.KEYWORD_ONLY,
    "delay_after": inspect.Parameter.KEYWORD_ONLY,
}

# 目标 API：select_multiple(items, mode=..., append=..., delay_after=...)。
# 公开 docstring 预留 fuzzy/exact；用户文档中的 regex 当前未接通。
# 当前实现直接抛 UnsupportedActionError（web.element.select_multiple）。
    ...
    try:
        ...
        return (
            _result(
                ...
                "已打开本次唯一 select_multiple 测试页面" if passed else "select_multiple 测试页面创建结果不完整",
                elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
...
def check_contract() -> dict[str, Any]:
    try:
        signature = inspect.signature(WebElement.select_multiple)
        ...
        items_required = (
            "items" in parameters
            and parameters["items"].default is inspect.Parameter.empty
        )
        ...
        return _result(
            ...
            items_required_ok=items_required,
...
def preflight_target(target_url: str, timeout: float) -> dict[str, Any]:
    ...
    try:
        ...
        return _result(
            "target_preflight",
            "PASS" if passed else "BLOCKED",
            "select_multiple 测试靶场可访问" if passed else "select_multiple 测试靶场未返回成功状态",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
    ...
    *,
    ...
    try:
        ...
        try:
            ...
            return _result(
                "select_multiple_fuzzy_items",
                status,
                "重置表单失败，无法开始 select_multiple",
                elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
                items=items,
                mode=mode,
                append=append,
        ...
        ret = select_element.select_multiple(
            items,
            mode=mode,
            append=append,
        ...
        print(
            f"select_multiple({items!r}, mode={mode!r}, append={append}, "
        ...
        return _result(
            "select_multiple_fuzzy_items",
            "PASS" if passed else "FAIL",
            "select_multiple 调用成功" if passed else "select_multiple 返回值不符合预期",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
            select_multiple_elapsed_ms=call_elapsed_ms,
        ...
        detail = "select_multiple 当前版本暂不支持（阻塞 VERIFIED）"
        if _safe_trace(exc) and _safe_trace(exc) != "web.element.select_multiple":
            detail = "select_multiple 被环境阻塞" if status == "BLOCKED" else "select_multiple 调用失败"
        if "暂不支持" in str(exc) or "unsupported" in str(exc).casefold():
            status = "FAIL"
            detail = "select_multiple 当前版本暂不支持（阻塞 VERIFIED）"
        print(
            f"select_multiple({items!r}, mode={mode!r}, append={append}) raised "
...
def _report(args: argparse.Namespace, results: Sequence[dict[str, Any]]) -> tuple[dict[str, Any], int]:
    ...
    excluded = [
        ...
        "cef",
        "auto",
        "mode=regex（公开 docstring 未接通；当前实现直接 unsupported）",
    ...
    return (
        {
            ...
            "excluded": excluded,
            "open_defects": ["web.element.select_multiple unsupported"],
        },
        exit_code,
    )
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="运行 uiautoma.web.WebElement.select_multiple() 持久化真实浏览器测试。"
    )
    parser.add_argument(
        "--mode",
        default="chrome",
    ...
    parser.add_argument(
        ...
        help=f"select_multiple 测试靶场，默认 {DEFAULT_TARGET_URL}",
    )
    parser.add_argument(
        "--element-library",
        type=Path,
