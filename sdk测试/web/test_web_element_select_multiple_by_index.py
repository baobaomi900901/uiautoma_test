"""WebElement.select_multiple_by_index() 在 iframe / Shadow 表单中的真实浏览器验收。
...
def check_contract(element_class: type) -> dict[str, Any]:
    ...
    expected = "select_multiple_by_index(self, indexes: list[int], *, append=False, delay_after=1) -> None"
    try:
        signature = inspect.signature(element_class.select_multiple_by_index)
        parameters = list(signature.parameters.values())
        ok = (tuple(signature.parameters) == ("self", "indexes", "append", "delay_after")
              ...
              and str(parameters[1].annotation) == "list[int]"
              and parameters[2].kind is inspect.Parameter.KEYWORD_ONLY
              and parameters[2].default is False
              and str(parameters[2].annotation) in {"bool", "<class 'bool'>"}
              and parameters[3].kind is inspect.Parameter.KEYWORD_ONLY
              and parameters[3].default == 1
              and str(parameters[3].annotation) in {"float", "<class 'float'>"}
...
def native_case(page: Any, case_id: str, indexes: list[int], expected_values: list[str],
                *, append: bool = False, defaults: bool = False, preselect: bool = False,
                keyword_indexes: bool = False) -> dict[str, Any]:
    started = time.perf_counter()
    expected = (f"{'预选北京；' if preselect else ''}select_multiple_by_index({indexes!r}, "
    ...
    try:
        ...
        if preselect:
            target.select_multiple_by_index([0], delay_after=0.15)
            if selected(page) != ["beijing"]:
                raise RuntimeError("北京前置选择未建立")
        invoked = time.perf_counter()
        if defaults:
            result = target.select_multiple_by_index(indexes)
        elif keyword_indexes:
            result = target.select_multiple_by_index(indexes=indexes, append=append, delay_after=0.1)
        else:
            result = target.select_multiple_by_index(indexes, append=append, delay_after=0.1)
...
def ant_boundary(page: Any, action_error: type) -> dict[str, Any]:
    started = time.perf_counter()
    expected = "Ant 库元素是 div；select_multiple_by_index([0,1]) 抛 ActionError/element_not_selectable，状态不变"
    try:
        ...
        try:
            result = target.select_multiple_by_index([0, 1], delay_after=0)
...
def invalid_case(page: Any, case_id: str, expected_type: type, fragment: str) -> dict[str, Any]:
    ...
    try:
        ...
        try:
            if case_id == "indexes_tuple":
                result = target.select_multiple_by_index((0, 1), delay_after=0)
            elif case_id == "indexes_mixed":
                result = target.select_multiple_by_index([0, "1"], delay_after=0)
            elif case_id == "indexes_bool":
                result = target.select_multiple_by_index([True], delay_after=0)
            elif case_id == "indexes_float":
                result = target.select_multiple_by_index([1.0], delay_after=0)
            elif case_id == "indexes_missing":
                result = target.select_multiple_by_index()
            elif case_id == "append_positional":
                result = target.select_multiple_by_index([0], True)
            else:
...
def run(args: argparse.Namespace) -> tuple[list[dict[str, Any]], int, str]:
    ...
    temp_dir = args.test_root / ".pytest_tmp" / "select_multiple_by_index" / uuid.uuid4().hex
    ...
    try:
        ...
        rows.append(native_case(page, "indexes_default", [0, 2], ["beijing", "guangzhou"], defaults=True))
        rows.append(native_case(page, "negative_indexes", [-1, -2], ["guangzhou", "shenzhen"]))
        rows.append(native_case(page, "unordered_indexes", [2, 0], ["beijing", "guangzhou"]))
        rows.append(native_case(page, "duplicate_indexes", [1, 1], ["shanghai"]))
        rows.append(native_case(page, "append_true", [2], ["beijing", "guangzhou"],
                                append=True, preselect=True))
        rows.append(native_case(page, "append_false", [2], ["guangzhou"], preselect=True))
        rows.append(native_case(page, "empty_replace", [], [], preselect=True))
        rows.append(native_case(page, "empty_append", [], ["beijing"], append=True, preselect=True))
        rows.append(native_case(page, "out_of_range_replace", [99, -99], [], preselect=True))
        rows.append(native_case(page, "out_of_range_append", [99, -99], ["beijing"],
                                append=True, preselect=True))
        rows.append(native_case(page, "mixed_valid_out_of_range", [99, 3, -99], ["shenzhen"]))
        rows.append(native_case(page, "keyword_indexes", [1], ["shanghai"], keyword_indexes=True))
        rows.append(invalid_case(page, "indexes_tuple", InvalidParamsError, "indexes"))
        rows.append(invalid_case(page, "indexes_mixed", InvalidParamsError, "indexes"))
        rows.append(invalid_case(page, "indexes_bool", InvalidParamsError, "indexes"))
        rows.append(invalid_case(page, "indexes_float", InvalidParamsError, "indexes"))
        rows.append(invalid_case(page, "indexes_missing", TypeError, "indexes"))
        rows.append(invalid_case(page, "append_positional", TypeError, "positional"))
    except Exception as exc:
        ...
        if native_ref is not None and package is not None:
            ...
            try:
                result = native_ref.select_multiple_by_index([0], delay_after=0)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="WebElement.select_multiple_by_index() iframe/Shadow 表单实测")
    ...
    report = {"api": "uiautoma.web.WebElement.select_multiple_by_index", "url": args.target_url,
