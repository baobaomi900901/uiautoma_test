"""uiautoma.web.WebElement.select_multiple_by_index() 持久化真实浏览器测试运行器。

导入本模块不会连接 Runtime、访问靶场或操作浏览器。真实场景打开唯一 Chrome
``form-controls`` 页面并连接指定元素库；对 ``select_html_多选`` 直接调用
``WebElement.select_multiple_by_index(...)``。当前版本显式暂不支持该 API。
"""
...
RUN_QUERY_KEY = "uiautoma_element_select_multiple_by_index_run"
...
EXPECTED_KINDS = {
    ...
    "indexes": inspect.Parameter.POSITIONAL_OR_KEYWORD,
    "append": inspect.Parameter.KEYWORD_ONLY,
    "delay_after": inspect.Parameter.KEYWORD_ONLY,
}

# 目标 API：select_multiple_by_index(indexes, *, append=..., delay_after=...)。
# 当前实现直接抛 UnsupportedActionError（web.element.select_multiple_by_index）。
    ...
    try:
        ...
        return (
            _result(
                ...
                "已打开本次唯一 select_multiple_by_index 测试页面" if passed else "select_multiple_by_index 测试页面创建结果不完整",
                elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
        ...
        return (
            _result(
                "owned_page_setup",
                "BLOCKED",
                "无法打开本次 select_multiple_by_index 测试页面",
                elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
                profile_directory=profile_directory,
                existing_session_reused=existing_session_reused,
                profile_identity_fallback_used=profile_identity_fallback_used,
...
def check_contract() -> dict[str, Any]:
    try:
        signature = inspect.signature(WebElement.select_multiple_by_index)
        ...
        indexes_required = (
            "indexes" in parameters
            and parameters["indexes"].default is inspect.Parameter.empty
        )
        ...
        return _result(
            ...
            indexes_required_ok=indexes_required,
...
def preflight_target(target_url: str, timeout: float) -> dict[str, Any]:
    ...
    try:
        ...
        return _result(
            "target_preflight",
            "PASS" if passed else "BLOCKED",
            "select_multiple_by_index 测试靶场可访问" if passed else "select_multiple_by_index 测试靶场未返回成功状态",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
        ...
        return _result(
            "target_preflight",
            "BLOCKED",
            "select_multiple_by_index 测试靶场不可访问",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
            **_error_fields(exc),
        )
...
def run_select_multiple_by_index_case(
    page: WebBrowser,
    ...
    *,
    ...
    try:
        ...
        try:
            ...
            return _result(
                "select_multiple_by_index_items",
                status,
                "重置表单失败，无法开始 select_multiple_by_index",
                elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
                indexes=indexes,
                append=append,
        ...
        ret = select_element.select_multiple_by_index(
            indexes,
            append=append,
        ...
        print(
            f"select_multiple_by_index({indexes!r}, append={append}, "
            f"delay_after={delay_after}) -> {ret!r}",
            file=sys.stderr,
            flush=True,
        )
        return_none_ok = ret is None
        passed = return_none_ok
        return _result(
            "select_multiple_by_index_items",
            "PASS" if passed else "FAIL",
            "select_multiple_by_index 调用成功" if passed else "select_multiple_by_index 返回值不符合预期",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
            select_multiple_by_index_elapsed_ms=call_elapsed_ms,
        ...
        detail = "select_multiple_by_index 当前版本暂不支持（阻塞 VERIFIED）"
        if _safe_trace(exc) and _safe_trace(exc) != "web.element.select_multiple_by_index":
            detail = (
                "select_multiple_by_index 被环境阻塞"
                if status == "BLOCKED"
                else "select_multiple_by_index 调用失败"
            )
        if "暂不支持" in str(exc) or "unsupported" in str(exc).casefold():
            status = "FAIL"
            detail = "select_multiple_by_index 当前版本暂不支持（阻塞 VERIFIED）"
        print(
            f"select_multiple_by_index({indexes!r}, append={append}) raised "
...
def _report(args: argparse.Namespace, results: Sequence[dict[str, Any]]) -> tuple[dict[str, Any], int]:
    ...
    excluded = [
        ...
        "原生 <select multiple>（当前元素为带 input 的城市多选控件）",
        "Ant/组件库下拉框",
    ...
    return (
        {
            ...
            "excluded": excluded,
            "open_defects": ["web.element.select_multiple_by_index unsupported"],
        },
        exit_code,
    )
        ...
        if scrub_result["status"] == "PASS":
            ...
            if page_result["status"] == "PASS" and page is not None:
                ...
                if (
                    ...
                    results.append(
                        run_select_multiple_by_index_case(
                            page,
                            select_element,
                            reset_element,
                            indexes=[1, 0],
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="运行 uiautoma.web.WebElement.select_multiple_by_index() 持久化真实浏览器测试。"
    )
    parser.add_argument(
        "--mode",
        default="chrome",
    ...
    parser.add_argument(
        ...
        help=f"select_multiple_by_index 测试靶场，默认 {DEFAULT_TARGET_URL}",
    )
    parser.add_argument(
        "--element-library",
        type=Path,
