"""uiautoma.web.WebElement.set_attribute() / get_attribute() 持久化真实浏览器测试运行器。

导入本模块不会连接 Runtime、访问靶场或操作浏览器。真实场景打开唯一 Chrome
``form-controls`` 页面并连接指定元素库；对 ``input元素`` 先 ``set_attribute`` 再
``get_attribute`` 往返核对；并单独验收 ``get_attribute`` 读取已有属性。
"""
...
DEFAULT_ATTR_NAME = "data-uiautoma-sdk"
DEFAULT_VALUE_PREFIX = "attr_"
RUN_QUERY_KEY = "uiautoma_element_attr_run"
...
EXPECTED_CONTRACTS = {
    "set_attribute": {
        ...
        "kinds": {
            ...
            "value": inspect.Parameter.POSITIONAL_OR_KEYWORD,
        },
        "required": ("name", "value"),
...
# 目标：set_attribute + get_attribute 往返；get 已有属性为对照验收。
    ...
    try:
        ...
        return (
            _result(
                ...
                "已打开本次唯一attribute 测试页面" if passed else "attribute 测试页面创建结果不完整",
                elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
...
def check_contract() -> list[dict[str, Any]]:
    ...
    for api_name, expected in EXPECTED_CONTRACTS.items():
        try:
            method = getattr(WebElement, api_name)
            signature = inspect.signature(method)
            parameters = signature.parameters
            order_ok = tuple(parameters) == expected["parameter_order"]
            defaults_ok = all(
                parameters[name].default == value for name, value in expected["defaults"].items()
            )
            kinds_ok = all(parameters[name].kind == value for name, value in expected["kinds"].items())
            no_extra_ok = set(parameters) == set(expected["parameter_order"])
            required_ok = all(
                name in parameters and parameters[name].default is inspect.Parameter.empty
                for name in expected["required"]
            )
            return_ann = signature.return_annotation
            return_ok = return_ann in expected["return"] or str(return_ann) in {
                str(item) for item in expected["return"]
            } or str(return_ann) in {"None", "NoneType", "<class 'NoneType'>", "str", "'str'"}
            if api_name == "set_attribute":
                return_ok = return_ann in {None, type(None), "None"} or str(return_ann) in {
                    "None",
                    "NoneType",
                    "<class 'NoneType'>",
                }
            if api_name == "get_attribute":
                return_ok = return_ann in {str, "str"} or str(return_ann) in {"str", "'str'"}
            passed = order_ok and defaults_ok and kinds_ok and return_ok and no_extra_ok and required_ok
            results.append(
                _result(
                    f"api_contract_{api_name}",
                    "PASS" if passed else "FAIL",
                    f"{api_name} 公开签名符合当前合同" if passed else f"{api_name} 公开签名与当前合同不一致",
...
def preflight_target(target_url: str, timeout: float) -> dict[str, Any]:
    ...
    try:
        ...
        return _result(
            "target_preflight",
            "PASS" if passed else "BLOCKED",
            "attribute 测试靶场可访问" if passed else "attribute 测试靶场未返回成功状态",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
...
def preflight_element_library(
    ...
    try:
        ...
        return _result(
            ...
            "PASS" if passed else "BLOCKED",
            "元素库包含 input元素 与 重置_html" if passed else "元素库缺少 input元素 或 重置_html",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
...
def connect_library_and_find_elements(
    ...
    try:
        ...
        passed = package.web_count > 0 and input_name_ok and reset_name_ok
        return (
            _result(
                "element_library_setup",
                "PASS" if passed else "FAIL",
                "已连接元素库并绑定 input元素 与 重置_html"
                if passed
...
def run_get_attribute_existing_case(input_element: WebElement) -> dict[str, Any]:
    started = time.perf_counter()
    try:
        call_started = time.perf_counter()
        value = input_element.get_attribute("placeholder")
        elapsed_ms = round((time.perf_counter() - call_started) * 1000, 1)
        print(f"get_attribute('placeholder')={value!r}", file=sys.stderr, flush=True)
        type_ok = isinstance(value, str)
        exact_ok = str(value) == "请输入文本"
        passed = type_ok and exact_ok
        return _result(
            "get_attribute_existing_placeholder",
            "PASS" if passed else "FAIL",
            "get_attribute 读取已有 placeholder 成功" if passed else "get_attribute 读取已有 placeholder 不符合预期",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
            get_attribute_elapsed_ms=elapsed_ms,
            return_type_ok=type_ok,
            value_exact_ok=exact_ok,
            expected_value="请输入文本",
            value=str(value),
            target_name=str(input_element.name or ""),
        )
    except Exception as exc:  # noqa: BLE001
        status = _error_status(exc)
        return _result(
            "get_attribute_existing_placeholder",
            status,
            "get_attribute 被环境阻塞" if status == "BLOCKED" else "get_attribute 调用失败",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
            **_error_fields(exc),
        )


def run_set_then_get_attribute_case(
    ...
    try:
        call_started = time.perf_counter()
        ret = input_element.set_attribute(attr_name, expected_value)
        set_elapsed_ms = round((time.perf_counter() - call_started) * 1000, 1)
        print(
            f"set_attribute({attr_name!r}, {expected_value!r}) -> {ret!r}",
            file=sys.stderr,
            flush=True,
        )
        verify = input_element.get_attribute(attr_name)
        print(f"verify get_attribute({attr_name!r})={verify!r}", file=sys.stderr, flush=True)
        return_none_ok = ret is None
        type_ok = isinstance(verify, str)
        exact_ok = str(verify) == expected_value
        passed = return_none_ok and type_ok and exact_ok
        return _result(
            "set_attribute_then_get_attribute",
            "PASS" if passed else "FAIL",
            "set_attribute 后 get_attribute 与写入值一致"
            if passed
            else "set_attribute/get_attribute 往返不符合预期",
            elapsed_ms=round((time.perf_counter() - started) * 1000, 1),
            set_attribute_elapsed_ms=set_elapsed_ms,
        ...
        status = _error_status(exc)
        # 当前产品显式 unsupported；记 FAIL 以阻塞 VERIFIED，而非环境 BLOCKED。
        detail = "set_attribute 当前版本暂不支持（阻塞 VERIFIED）"
        if _safe_trace(exc) and _safe_trace(exc) != "web.element.set_attribute":
            detail = "set_attribute 被环境阻塞" if status == "BLOCKED" else "set_attribute 调用失败"
...
def _report(args: argparse.Namespace, results: Sequence[dict[str, Any]]) -> tuple[dict[str, Any], int]:
    ...
    excluded = [
        ...
        "cef",
        "auto",
        "自定义属性之外的 aria/style 边界",
        "set_attribute 实现后的负例矩阵",
        "元素库捕获期 WebSessionId 在不清理时仍可直连当前页面（产品侧会话绑定）",
    ]
    return (
        {
            "api": ["uiautoma.web.WebElement.set_attribute", "uiautoma.web.WebElement.get_attribute"],
            "status": status,
            "exit_code": exit_code,
            "mode": args.mode,
            ...
            "open_defects": ["web.element.set_attribute unsupported"],
...
def run(args: argparse.Namespace) -> tuple[dict[str, Any], int]:
    results = list(check_contract())
    if any(item["status"] != "PASS" for item in results) or args.contract_only:
    ...
    try:
        ...
        if scrub_result["status"] == "PASS":
            ...
            if page_result["status"] == "PASS" and page is not None:
                library_result, package, input_element, reset_element = (
                ...
                if (
                    ...
                    results.append(run_get_attribute_existing_case(input_element))
                    results.append(
                        run_set_then_get_attribute_case(
                            input_element,
                            args.attr_name,
                            args.expected_value,
                        )
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="运行 WebElement.set_attribute/get_attribute 持久化真实浏览器测试。"
    )
    parser.add_argument(
        "--mode",
        default="chrome",
    ...
    parser.add_argument(
        ...
        help=f"attribute 测试靶场，默认 {DEFAULT_TARGET_URL}",
    )
    parser.add_argument(
        "--element-library",
        type=Path,
        default=DEFAULT_ELEMENT_LIBRARY,
        help="包含 input元素 与 重置_html 的元素库目录。",
    )
    parser.add_argument(
        "--input-element-name",
    ...
    parser.add_argument(
        "--attr-name",
        default=DEFAULT_ATTR_NAME,
        help=f"往返设置的属性名，默认 {DEFAULT_ATTR_NAME}。",
    )
    parser.add_argument(
        "--expected-value",
        default="",
        help="写入并期望读回的值；默认自动生成随机字符串。",
    )
    ...
    args.attr_name = str(args.attr_name or "").strip()
    args.expected_value = str(args.expected_value if args.expected_value is not None else "").strip()
    if not args.attr_name:
        parser.error("--attr-name 不能为空")
    if not args.expected_value:
        args.expected_value = f"{DEFAULT_VALUE_PREFIX}{uuid.uuid4().hex[:12]}"
