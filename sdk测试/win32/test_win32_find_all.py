r"""``uiautoma.win32.Win32Window.find_all()`` 持久化真实元素库测试。

API 参数（不是测试脚本参数）::

    Win32Window.find_all(
...
* ``timeout``：仅限关键字，单位秒；默认 ``20``，``0`` 只查一次，正数在期限内
  ...
  ``InvalidParamsError``。
* 返回值：始终为 ``list[Win32Element]``；超时仍未找到时返回空列表。

测试脚本参数：无。

脚本借用 UIAutoma 当前启用的 ``D:\code\元素库\260902_win元素``，在用户已经
启动的 Win32 靶场窗口中查找 ``win32靶场_表单控件_输入框_姓名``。覆盖名称字符串、Selector
对象、默认/零/正数/无限超时、未命中空列表以及非法参数。

当前元素库中的该选择器是唯一抓取项，因此真实场景验证返回列表与单项结果，但不
宣称已验证“同一选择器命中多个实时元素”。测试结束后只关闭借用的 Package 连接，
不关闭或修改 Win32 靶场程序。导入本模块不会连接 Runtime。
...
MISSING_ELEMENT = "__uiautoma_find_all_missing_element__"
...
def _check_contract() -> dict[str, Any]:
    ...
    try:
        signature = inspect.signature(Win32Window.find_all)
        ...
        return _result(
            ...
            (
                "公开签名为 find_all(selector: str | Selector, *, timeout=20)"
                if passed
                else "公开签名与当前合同不一致"
            ),
            elapsed_ms=_elapsed(started),
...
def _match_info(matches: object) -> dict[str, Any]:
    ...
    elements = [item for item in matches if isinstance(item, Win32Element)]
    runtime_ids = [str(item.id) for item in elements]
    source_ids = [str(item.raw.get("source_element_id") or "") for item in elements]
    names = [str(item.name or "") for item in elements]
    return {
        "is_list": True,
        "count": len(matches),
        "all_elements": len(elements) == len(matches),
...
def _run_match_case(
    ...
    *,
    ...
    try:
        matches = (
            window.find_all(selector)
            if timeout is None
            else window.find_all(selector, timeout=timeout)
        )
        info = _match_info(matches)
        passed = (
            info["is_list"]
            and info["count"] == 1
            and info["all_elements"]
            and all(value.startswith("rt:win:") for value in info["runtime_ids"])
            and info["source_ids"] == [source_id]
            and info["names"] == [TARGET_ELEMENT]
        )
        return _result(
            case_id,
            "PASS" if passed else "FAIL",
            success_detail if passed else "返回列表、元素数量或 Runtime 元素身份不符合预期",
            elapsed_ms=_elapsed(started),
        ...
        return _error_result(case_id, "find_all() 调用失败", started, exc)
...
def _run_empty_case(
    ...
    *,
    ...
    started = time.perf_counter()
    try:
        matches = window.find_all(MISSING_ELEMENT, timeout=timeout)
        elapsed_ms = _elapsed(started)
        passed = isinstance(matches, list) and not matches
        if timeout > 0:
            passed = passed and elapsed_ms >= timeout * 800
        return _result(
            case_id,
            "PASS" if passed else "FAIL",
            success_detail if passed else "未命中结果、返回类型或等待时长不符合预期",
            elapsed_ms=elapsed_ms,
            timeout=timeout,
            return_type=type(matches).__name__,
            count=len(matches) if isinstance(matches, list) else None,
        )
    except Exception as exc:  # noqa: BLE001
        return _error_result(case_id, "未命中查询抛出异常", started, exc)
...
def _print_case(result: dict[str, Any], index: int, total: int, *, color: bool) -> None:
    ...
    if result.get("count") is not None:
        print(f"{indent}{_pad('实际数量', 10)}: {result['count']}")
    ...
    if window is not None and selector is not None:
        ...
        results.extend(
            [
                _run_match_case(
                    "string_default",
                    "元素名称与默认 timeout=20 返回 1 个 Runtime 元素",
                    window=window,
                ...
                _run_expected_errors(
                    ...
                    [
                        ("列表", lambda: window.find_all([TARGET_ELEMENT], timeout=0)),  # type: ignore[arg-type]
                        ("整数", lambda: window.find_all(123, timeout=0)),  # type: ignore[arg-type]
                    ],
                ),
                _run_expected_errors(
                    "wrong_framework",
                ...
                _run_expected_errors(
                    ...
                    [
                        ("小于 -1", lambda: window.find_all(selector, timeout=-2)),
                        ("非数字", lambda: window.find_all(selector, timeout="bad")),  # type: ignore[arg-type]
                    ],
                ),
            ]
        )
