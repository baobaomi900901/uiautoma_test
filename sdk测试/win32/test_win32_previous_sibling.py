r"""Win32Element.previous_sibling() 实测。

API 参数（不是脚本参数）::

    element.previous_sibling() -> Win32Element

无公开参数，内部默认预算 5 秒。返回同一父元素下 UIA 顺序中的前一个兄弟，
不是视觉上的左侧控件。无前一个兄弟时预期 ElementNotFoundError。

脚本参数：无。运行：uv run .\win32\test_win32_previous_sibling.py
...
def check_contract():
    sig = inspect.signature(Win32Element.previous_sibling)
...
def check_result(actual, expected, reference):
    ...
    assert actual.raw.get("relation") == "previous_sibling", f"关系错误：{actual.raw.get('relation')}"
...
def main():
    ...
    def previous(index):
        actual = reference[index].previous_sibling()
        detail = check_result(actual, reference[index - 1], reference)
        return f"第 {index} 项的前驱与第 {index - 1} 项一致：{detail}"

    def chained():
        first = reference[-1].previous_sibling()
        check_result(first, reference[-2], reference)
        second = first.previous_sibling()
        detail = check_result(second, reference[-3], reference)
        return f"末项连续两次 previous_sibling() 返回倒数第三项：{detail}"

    def no_previous():
        try:
            result = reference[0].previous_sibling()
        except ElementNotFoundError:
            return "首项无前一个兄弟，正确抛出 ElementNotFoundError"
        raise AssertionError(f"预期 ElementNotFoundError，实际返回 {result!r}")

    def extra_args():
        item = reference[1]
        for call in (lambda: item.previous_sibling(1),
                     lambda: item.previous_sibling(timeout=1)):
            try:
                call()
            except TypeError:
                continue
    ...
    print(f"  API     : uiautoma.win32.Win32Element.previous_sibling\n  元素库  : {LIBRARY}")
    print(f"  面板元素: {PANEL}\n  顺序参照: children() 的当次结果，索引从 0 开始\n")
    try:
        run("API 合同", check_contract)
        run("表单页与参照准备", prepare)
        cases = [("第二项前驱", lambda: previous(1)),
                 ("中间项前驱", lambda: previous(len(reference) // 2)),
                 ("最后项前驱", lambda: previous(len(reference) - 1)),
                 ("链式前驱", chained), ("首项无前驱", no_previous),
                 ("参数数量限制", extra_args)]
