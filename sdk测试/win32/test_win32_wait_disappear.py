r"""``uiautoma.win32.Win32Window.wait_disappear()`` 持久化真实元素库测试。

API 参数（不是测试脚本参数）::

    Win32Window.wait_disappear(
...
* ``timeout``：位置或关键字参数，单位秒；默认 ``20``，``0`` 只查一次，正数在
  ...
  ``InvalidParamsError``。
* 返回值：超时前元素消失返回 ``True``，超时后仍存在返回 ``False``；所属窗口
  已关闭也视为元素消失。
...
正数/无限超时、已存在/不存在状态和非法参数。

本轮不修改靶场来制造“等待期间从存在变为消失”，也不关闭靶场验证窗口关闭分支。
``timeout=-1`` 只对确认不存在的名称调用，因此会立即返回，不会让脚本永久挂起。
测试结束后只关闭借用的 Package 连接。导入本模块不会连接 Runtime。
...
MISSING_ELEMENT = "__uiautoma_wait_disappear_missing_element__"
FINITE_PRESENT_TIMEOUT = 0.5
...
CASE_LABELS = {
    ...
    "target_window": "靶场窗口",
    "string_present_zero": "名称目标仍存在",
    "selector_present_zero": "Selector 仍存在",
    "element_present_zero": "Win32Element 仍存在",
    "missing_default": "默认超时已消失",
    "present_finite": "有限等待仍存在",
    "missing_infinite": "无限等待值",
    "missing_zero": "零超时已消失",
...
def _check_contract() -> dict[str, Any]:
    ...
    try:
        signature = inspect.signature(Win32Window.wait_disappear)
...
def _run_wait_case(
    ...
    *,
    ...
    try:
        if timeout is None:
            returned = window.wait_disappear(target)
        elif timeout_as_position:
            returned = window.wait_disappear(target, timeout)
        else:
            returned = window.wait_disappear(target, timeout=timeout)
        elapsed_ms = _elapsed(started)
        passed = returned is expected and elapsed_ms >= minimum_elapsed_ms
        return _result(
            case_id,
            "PASS" if passed else "FAIL",
        ...
        return _error_result(case_id, "wait_disappear() 调用失败", started, exc)
...
def _run_element_case(window: Win32Window, selector: Selector) -> dict[str, Any]:
    ...
    try:
        ...
        element = matches[0]
        returned = window.wait_disappear(element, timeout=0)
        passed = returned is False
        return _result(
            "element_present_zero",
            "PASS" if passed else "FAIL",
            (
                "已有 Win32Element 被转换为源 Selector，元素仍存在并返回 False"
                if passed
                else "Win32Element 仍存在时未返回 False"
            ),
            elapsed_ms=_elapsed(started),
            returned=returned,
        ...
        return _error_result("element_present_zero", "Win32Element 参数测试失败", started, exc)
    ...
    if window is not None and selector is not None:
        results.extend(
            [
                ...
                _run_wait_case(
                    "selector_present_zero",
                    "Selector 仍存在，timeout=0 返回 False",
                    window=window,
                    target=selector,
                    timeout=0,
                    expected=False,
                ...
                _run_wait_case(
                    "missing_default",
                    "不存在名称使用默认 timeout=20 立即返回 True",
                    window=window,
                ...
                _run_wait_case(
                    "present_finite",
                    "现存元素在 0.5 秒内未消失并返回 False",
                    window=window,
                    target=TARGET_ELEMENT,
                    timeout=FINITE_PRESENT_TIMEOUT,
                    expected=False,
                    timeout_as_position=True,
                    minimum_elapsed_ms=FINITE_PRESENT_TIMEOUT * 800,
                ),
                _run_wait_case(
                    "missing_infinite",
                    "不存在名称使用 timeout=-1 立即返回 True",
                    window=window,
                    target=MISSING_ELEMENT,
                    timeout=-1,
                    expected=True,
                ...
                _run_expected_errors(
                    ...
                    [
                        ("列表", lambda: window.wait_disappear([TARGET_ELEMENT], 0)),  # type: ignore[arg-type]
                        ("整数", lambda: window.wait_disappear(123, 0)),  # type: ignore[arg-type]
                    ],
                ),
                _run_expected_errors(
                    "wrong_framework",
                ...
                _run_expected_errors(
                    ...
                    [
                        ("小于 -1", lambda: window.wait_disappear(selector, -2)),
                        ("非数字", lambda: window.wait_disappear(selector, "bad")),  # type: ignore[arg-type]
                    ],
                ),
            ]
        )
